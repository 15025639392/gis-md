...
| **T-V11** | 共享位移模板 VBO 内存有界(不随浏览历史单调增长) | 【机制】 | ✅ 代码已落地(2026-08-26,`e7c47937`);真机内存观测待补 | 根因=`cache_`(`std::unordered_map<key, Entry>`)全文件零 `erase`/`clear`/容量上限,每浏览一个新 `{z,row}` 永久 +132KB(coarse 65²)或 +2.02MB(dense 257²);z7–z12 共 8064 行,一次全球浏览即 ~118MB(coarse)以上。修法=换成分档槽位池(`TerrainPageLayerPool` + generation 失效):淘汰释放 `vertexBuffer` 并 `totalVertexBytes_ -= old.vertexBytes`,容量 coarse 128 / dense 16。**host 自证**:`residentTemplateCount()`/`totalVertexBytes()` 由池容量硬封顶(coarse 132KB×128≈16.9MB + dense 2.02MB×16≈32.3MB),淘汰有明确减回路径 | 上限 ≈ 49MB(16.9 + 32.3);淘汰重入视野的模板重建成本见 **T-P8** |
...
| **T-P8** | 模板 VBO 淘汰后重建(随 T-V11 引入):槽位池满时离屏久驻模板被淘汰,其 `vertexBuffer` 释放;重新入视野时重建(`buildTerrainDisplacementTemplate` + 打包 + `createBuffer`)。dense 单片 ~18ms(引用 T-P7 实测的模板构建分量),coarse 更小 | **未量化频率**;容量 coarse 128 / dense 16 相对峰值可见 `{z,row}` 数(coarse ~15-25 / dense ~3-10,实测峰值可见瓦片 103)留有余量,预期淘汰只落在离屏久驻瓦片、重入视野一次性重建,非逐帧 thrash。真机验收 T-V11 时一并观测:拖动+缩放浏览大片区域,若 `templateBuildLogged` 日志在稳态后仍高频打出(而非仅首见/换档)则说明容量不足,需上调容量或加 LRU 预热 |
