...
| **T-V10** | 瓦片边界不出现发白细线(网格状,竖+横) | 【观感】 | ⚠️ **一处根因已定位并修**(2026-08-16,host 守卫双向验证 82.28°→<3°);**观感未验** —— 转 ✅ 需真机 A/B:同机位对 `docs/assets/tv10/vline_repro_1p69Mm.png` 的标定点(x=363)重跑 `seam_line_detect.py`,6.1σ 掉到不显著 | 见下方「T-V10 交接」的「根因①」 | 0(烘焙内多几处有效性分支) |
...
⚠️ 未结:①观感是否真的消失只能真机判;②修前的量级(82°法线错)与屏上仅 6 亮度单位
的对比度之间有落差,合理解释是高空大气/雾把地表对比整体压掉,**未证**;③若真机上线
只是变淡未消失,说明还有根因②,下一嫌疑仍是边界节点短臂(reach=0.5px vs 网格 8px)
带来的频段不一致,那是数据侧属性(切瓦多带几圈环)不是引擎单方面能消的。
