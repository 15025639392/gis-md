...
用法与 `vector.md` 同:判据编号 `V*` / 性能债编号 `P*` 是**跨会话稳定引用锚点**
...
| # | 判据 | 类型 | 状态 | 代价 | 证据 |
|---|---|---|---|---|---|
| **V1** | 目标层级源瓦全缺时,该瓦显示**祖先影像**而非空白 | 【观感】 | ⚠️ 机制已通,像素未判 | 0(复用祖先纹理 + UV 窗口,`uploadCount == 0`) | 见下方「V1/V2 机制证据」 |
| **V2** | 一块空洞瓦**不得毒化其子孙链** —— 子孙应越过它取到更高层可画祖先 | 【机制】 | ✅ 达成 | 同上 | 同下 |
| **V3** | 空洞瓦不驱动细化(refine 进洞只会得到更多洞) | 【机制】 | ✅ 达成 | 0 | `originalFailed_` 置位 → `MoreDetail::No` |
| **V4** | 退化不得引入逐帧重合成 / 重下载 | 【机制】 | ✅ 达成 `推断` | 0 网络 | 空瓦按 cacheKey 命中既有 tile;`loadMappedRasterTile` 对 Loaded 早退。**未做长时真机计数验证** |
| **V5** | 空洞区的边界不应出现"半张图"错缝 | 【观感】 | ❌ 未验 | — | 无复现场景,待你给 |

### V1/V2 机制证据(2026-08-15)
...
> **V1 仍是 ⚠️ 而非 ✅**:机制我自证了,但"空洞区显示模糊父级影像"好不好看是
...
| # | 判据 | 类型 | 状态 | 代价 | 证据 |
|---|---|---|---|---|---|
| **V6** | 影像瓦走共享 HTTP 缓存,内存缓存逐出后回摇不重下 | 【机制】 | ✅ 达成 | — | commit `7f86e76b5`(P1 结清);WMS/WMTS/Bing/Google/TMS 均继承 `XYZImageryProvider` 的缓存路径 |
| **V7** | 缓存条目可过期 / 可重验(ETag、304),陈旧影像不会永久钉死 | 【机制】 | ❌ 未做 | — | `HttpCache::expiryTime` 全仓**只有定义没有生产赋值**,见 P1 |
| **V8** | 失败瓦不被反复请求 | 【机制】 | ✅ 达成 | 每条失败记忆 1 字节配额 | `cacheTerminalFailure` 写 `terminalFailure` 条目,查找侧当命中短路 |
...
| # | 判据 | 状态 | 证据 |
|---|---|---|---|
| **V9** | Google Maps Tiles ToS 要求的归属 logo 显示 | ❌ 未做 | `GoogleMapTilesImageryProvider.cpp` 的 TODO;曾有的 `showLogo` 占位位(恒 true、无消费者)已于 2026-08-07 删除 |
| **V10** | WMS 版本适配:`version` 可配但只实现了 1.3.0 口径 | ⚠️ 有缺口 | 无条件写 `crs=EPSG:4326` + lat,lon 轴序;全文件无 `srs` / 1.1.1 分支 → 配 1.1.1 会同时错在参数名和轴序 |
...
| 项 | 我的建议 | 锚点 |
|---|---|---|
| 空洞区显示模糊父级影像 vs 干净空白 | 我押父级影像(用户读作"精度不够"而非"坏了"),但这是像素判断,归你 | V1 |
| `HttpCache` 过期口径(固定 TTL?尊重源端 Cache-Control?) | 建议先做 ETag/304 重验,TTL 次之 —— 重验不猜时长 | V7 / P1 |
| WMS 1.1.1 是否要支持 | 我押支持(纯逻辑缺口,可单测钉死,改动面小) | V10 |
| Google 归属 logo 是否要做 | 取决于你是否真用 Google 源发布 | V9 |
