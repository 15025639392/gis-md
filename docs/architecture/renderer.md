...
> **删除提示：历史 MVT 道路线场已完整物理删除。** `TerrainPageStore` 不再包含路网 SDF/D2 场页，相关纹理槽、uniform、shader 与平台绑定也已删除。
...
### 7. 地形页存储在渲染侧的角色
`TerrainPageStore` 是地形表面影像的常驻纹理池:把地形瓦切成 `gridN²` cell,每 cell 独立按 LRU 拿 `texture2DArray` 一层,着色器经间接纹理查层号,取代"每瓦片一张合成影像"。渲染帧内 `updateVisiblePages` 每帧驱动(枚举可见 cell→缺页 fetch→写间接纹理),`applyToTerrainCommand` 把层号/页参数写进地形 `RenderCommand`。现行槽位只有影像页及独立的地形高度纹理；已删除的 road-field 槽位不属于 ABI。
...
- **地形页存储扩展**:现行 PageStore 只承载影像页；地形高度纹理由独立高度资源链管理。新增页化平面必须建立新的完整合同与同步守卫，不能恢复已删除的 road-field ABI。
