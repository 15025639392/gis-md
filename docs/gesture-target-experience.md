...
- 判据在起手时判定一次，**拖拽中途不切换**（防止画面语义突变）：
  - 近地拖图（strafe）：相机海拔 < 150,000m **且** 起手拾取射线与地表**掠射**
    （`|dot(rayDir, 地表法线)| < 0.05`，Cesium `tangentPick` 判据，约地平线 3° 内）
    或相机在拾取点内侧；
  - 否则空间拖球（绕地心旋转，拾取点钉在指下）。
...
- 平移模型 = **Cesium strafe**（2026-08-22 对齐 Cesium）：平面 = 过拾取点、
  法线 = **相机视线方向**（不是地表法线）——视线法平面在地平线处不退化，
  故**无地平线钳制/预算**，地图自由跟手；锚点像素误差 ≤ 1px。
- 惯性：方向锁定、指数衰减（与空间惯性同规则），<0.5px/帧停，**无边界**。
...
| 轴 | 手势 | 阈值 | 输出 |
|---|---|---|---|
| 缩放 | 两指距离变化 | 0.1 log2（≈7.2% 距离变化） | 绕**屏幕中心拾取点**对数缩放（Cesium zoom3D 同款锚点） |
| 旋转 | 两指连线转角 | 25px 弧长自适应（角度阈值 = 25/(π·d)·360°） | 绕锚点法线旋转；无锚时回退**绕指下地表点竖直轴旋转**（地图旋转语义，奥维同款：锚点钉住、地平线水平、天空不转） |
| 倾斜 | 两指同向竖移 | 起手竖直判定 | 0.5°/px（保留质心绝对值映射；真机微调） |
| 平移 | 双指整体移动 | 随动 | 锚点刚性平移 |

- 缩放/旋转绕指下锚点，锚点像素误差 ≤ 1px（与单指一致）；高倾斜/无锚时旋转退化为绕指下地表点竖直轴（锚点钉住、地平线水平——绕质心射线滚转会连带天空一起转，绕相机竖直轴则聚焦屏幕中心而非双指中心）。
- 任一手指抬起 → 双指结束，立即停。
- 新手势打断旧惯性（单指/双指互抢）。
...
- rotate（2026-08-23 起）：**保留惯性**——松手按拧动速率滑行（有锚绕锚点法线、无锚绕指下地表点竖直轴），指数阻尼 2/s（Cesium `inertiaSpin=0.9` 同量级）、0.5px/帧感知判停（Cesium 规则），速率 ≥0.05 rad/s 才启动；cancel 一律即停。
- tilt：无惯性。
...
- 双指：组合动作（边捏边转边斜）无串扰、无跳变；倾斜满倾角约 150px 竖移；zoom 惯性永不越过锚点；rotate 惯性单调衰减停（0.5px/帧停）；pan 松手即停，cancel 全部即停。
...
3. 近地平移 strafe 平面法线取视线（Cesium 同款，2026-08-22）；缩放锚点取屏幕中心拾取点（Cesium zoom3D 同款）；无地平线钳制后无需边界参数；
