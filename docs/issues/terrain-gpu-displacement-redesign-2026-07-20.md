...
> **📍 进度快照（2026-07-21）**：P0 ✅ / P1a ✅ / P1b ✅（commit 81aefb818+365711394+479d4d4a7，Stage A 贴椭球→Stage B 高度纹理 shader 位移，真机 relief 确认：同相机 A/B 一致 + 掠视 30.9→60fps）/ **P2 ✅（commit deb97c702，见下，morph 连续生长）** / GPU 路径无高度图瓦片回落 guard ✅（commit 7f747bbc2，预防上采样变平 + 删过时 Stage-A）。**几何侧地形生长（z0–z12 shader 位移 + 距离连续 morph）功能上已完整。** P3 ✅（`d6c2c1738`,高度查询迁 DecodedHeightmap,host 验证）/ P4 ✅（`3f35ed3a8`,skirt+浮雕法线+overlay 回归,真机自验,见下）。**P5a ✅ default-on(`aa1ab715d`) / P5b 基建 ✅ skip flag 默认关(`73274ac30`,影像启动链竞态专项修复后开启,见 §6 P5)**。⚠️ 残留:demo GPU ON→OFF 重瓦片视图 pool.reset() 悬垂句柄崩溃（demo-only 低优先）。
...
**P3 高度查询迁移 ✅ 代码完成 host 验证(commit `d6c2c1738`,2026-07-23)**：`LoadedTerrainHeightSampler` 叶子从「遍历 glTF 三角网格」改为「直接采 per-tile `retainedHeightmap` + `DecodedHeightmapSampler`」(后者已就绪零调用点),删 glTF 三角遍历 ~150 行;扫描骨架(bounds.contains+z降序+祖先兜底)原样保留,无 retainedHeightmap 候选跳过→回落覆盖祖先。**纯替换非混合**:GPU 位移下 glTF 是平模板(遍历得椭球 ~0 错值),故 glTF 采样路径不能与 default-on 共存必删;retainedHeightmap 位移前后都决定渲染几何→采它两态都对。矢量贴地当前零调用点(LoadedTerrainAreaSampler 备而未用)。测试:DecodedHeightmapSampler 新单测(插值17.5/越界/no-data/invalid)+两既有测试 heightmap 化(保 nullopt/最深/祖先/同级最高;删 glTF 专用 strip/fan/堆叠/transform 测)+test_heightmap_terrain 集成测试补 setRetainedHeightmap(真实解码链路→retainedHeightmap 采样得 640m=反证设计)。host 零新回归(3 既存失败 sse_pipeline/raster_overlay_details/scene_frame_state 与本改无关,HEAD 同 FAIL)。**⏳ 待真机验证**:相机贴地/拾取,尤其 GPU 位移 ON 下(平模板世界高度查询仍工作=P3 使能 default-on 的决定性证明,与 P5 一并验最省)。
**P4 skirt/法线/normal-map + overlay 回归 ✅ 完成（commit `3f35ed3a8`,2026-07-23,真机自验）**：三块——
- **skirt ✅**（早前 `1f6f99074`+`94a0cfd08`+`c3985ef02`）:裙顶点 heightDelta=-1 哨兵→shader 跳位移停椭球面,撑成逐瓦片自适应墙(墙高=边缘真实高度),根治粗 LOD 24-385km 巨墙 + chevron 尖刺。
- **浮雕法线 ✅**（本次）:terrain fragment 着色法线从光滑椭球法线(`v_normal`)改为**位移面真实几何法线** `cross(dFdx(v_position),dFdy(v_position))`,用 `v_normal` 定向保证朝外,平坦处自然退化=改前完全一致(no-op)。两后端(GLSL+MSL)对称,**零新 uniform/CPU/模板改动**——因 `v_position` 已是位移后局部米坐标、`u_lightDir` 已在同 ENU 帧(`SceneRenderCommandUniformUpdater` 位移帧路径把 light/eye 变到局部帧)。选 position 屏幕导数而非顶点纹理邻居梯度=避开 meters-per-cell uniform 的 Metal 顶点 buffer plumbing,更手术式。**真机自验(Adreno 7e045e39)**:①debug 法线可视化(法线当颜色)证法线随地形起伏变化=机制生效;②冷启动同姿态 A/B 像素差分 mean 0.24/>8 变化 0.0%(重庆 1500m/45° 斜视此处地形坡缓→法线近朝上→**观感零变化**);③glError=0 零回归零崩溃。**结论**:正确+免费+只增不减(平地=改前,陡山=真实 hillshade),但 demo 默认视角观感无感=印证文档「normal-map 归观感提升」。⚠️ 残留:debug 图显示逐面 faceting,平缓处被 shaded 掩没,**陡地形若显现则升级顶点梯度平滑法线(需 meters-per-cell uniform)**。
- **overlay 回归 ✅**:位移纯径向(沿法线)→顶点经纬水平位置不动→overlay UV 不漂;真机影像贴合无回归(P3 及本次多次冷启动截图坐实)。
**P5 收尾 — P5a ✅ / P5b 基建 ✅(flag 默认关,commit `aa1ab715d`+`73274ac30`,2026-07-23)**：
- **P5a default-on ✅**（`aa1ab715d`）:引擎默认 `terrainGpuDisplacementEnabled_=true`,删 demo 显式调用;**关键=pool 创建移到 render() 内 scene update 之前**(swap 在命令 build 定型,pool 晚建则首帧命令缓存成 CPU 路径永不生效)。真机插桩证 default-on 模板 swap 生效(z=12)。
- **P5b skip 基建 ✅ flag 默认关**（`73274ac30`）:对「必走模板」fine 瓦片跳过废弃 per-tile VBO 建/传(根治 terrainUpload 尖刺+每瓦片省 ~538KB)。机制真机验证(z=12 skip/coarse 与 z13+ 上采样保留 legacy/z=7 过渡带正确);`isValidGpuReadyPrimitive` 需按 sharedTemplateGeometry 放行(踩坑:空 bytes 判无效→markFailedTemporarily 重试风暴)。**⚠️ 为何默认关**:skip 让几何单帧内全部 Done,暴露**影像启动链竞态死锁**(mapping→fetch→attach 隐含以「几何加载慢、多帧非 reuse」为前提;三个串联 hold 源:tileset imagery-hold / takeover-hold / command-build-hold)→偶发首屏黑屏永 hold。修好影像启动链专项后 `Renderer::setTerrainBakedVboSkipEnabled(true)` 即启用。
- **顺手捞出的独立真 bug(已修,默认生效)**:strict-reuse 启动死锁——几何单帧全 Done→revision 恒定→立即 strict reuse→overlay prefetch 永不再跑→影像请求永不发出→hold 永不释放。修=`TileSelectionReuseInput::presentationHeld` 首屏未达成期间禁 reuse(与 TilesetUpdateFrameRuntime 顶部 async bootstrap 死锁同型;legacy 时序下也是潜在死角)。
- 残留:影像启动链竞态专项→flag 默认开→legacy 路径退役+golden 重建(selector trace 预期无 diff,渲染无像素 golden,主要是 `test_terrain_shader_command` 断言);AI_INDEX 更新。
