...
- **Step B2b — 接线(下一步)**:池改页粒度(`blockLayers=1`,key=页)+ 只 fetch 可见页 → 从 resident 页建**稀疏**间接纹理(miss→alpha=0→回落 mappedRaster)+ 顺带收紧 thick-OBB 过取。真机 gate=近景 crisp(z17 真实影像)+内存有界+无洞。
