...
- **Step 3**:terrain 片元从 `sampler2DArray` 按页表 layer 采样,一个 capped 粗瓦片显示正确高清影像,验①路径通 ②边界 = 现状;**并顺带量 Step 2 的 ghost**(拖动中对 live array 持续 texSubImage,看是否 rename/stall)。拆两刀:
  ...
  - **Step 3b ✅ 代码已落地 + 真机验收 PASS(commit `d86f32157`)**:页填充换真实更深 raster 影像。目标瓦片锁定后异步拉它更深 LOD(z + `depthLevels`=z+2 → `gridN`=4=16 片)的覆盖影像:`ImageryProvider::requestTile`(从 `overlays.front()->getTileProvider()` 捕获 provider)→ worker 回调投 shared inbox(shared_ptr 存活于回调,`TerrainPageStore` 析构不悬垂)→ 渲染线程 `tick()` 排空 → 逐层 `updateTextureRegion(layer)`。`gridN`=2^`depthLevels` 在 web-mercator 同分块下把目标瓦片铺成对齐子瓦片网格(`scheme.positionToTile(cell 中心, zoom)` 枚举);影像行序按 XYZ→terrain UV 翻转(`kFlipRowsOnUpload`),RGB/RGBA/灰度归一化 RGBA 上传;非 256² 跳过留占位;合成图案降级为占位。**真机(Adreno 7e045e39)验收 PASS**:整屏(全挂诊断)显真实 z14 卫星纹理(河道/城区/路网清晰、朝向正确、16 层间**无页缝**=CLAMP_TO_EDGE 生效),15/16 层灌真实影像 = capped z12 几何瓦片经 array 路径显 z14 真实影像(**①路径通,影像细于几何 LOD**)。
    - **未尽**:①ghost(§12.5#6)一次性填充未激发,需拖动中 live 换页才量(生产 SVT 页 swap 场景);②单瓦片可见性受"目标=SSE 最大瓦片=nadir"影响(顶视相机下离屏,单瓦片模式此相机看不到;全挂诊断已证路径),换前视相机或指定 key 可让单瓦片上屏;③UV 目前是线性栅格近似(mercator 非线性在 z12 瓦片内可忽略),生产需精确子瓦片 UV。
- 任一步卡住 → 退 2D atlas + per-page clamp(§12.6③ 原计划,无损)。**三步全过 → 合成方案(CPU 驱动 SVT + texture2DArray)生产原型端到端打通(创建/上传/门控/采样/真实影像 fetch 全真机验)。**
...
- **Step B2a ✅ 门② determination 接相机 + 真机验证(commit `7735903c0`)**:可见页 walk 挪进选择帧(Engine post-`scene_->update()` pre-`tick()`,取 `frameState().selectorViews[0]`+`tilePlan().tilesToRenderThisFrame`+tileset overlays provider)。纯读+插桩,**不碰池/fetch/render**(B1 dense 照跑)。门② 数学全复用现成(`boundingRegionObb`+`Frustum::intersectsOBB`+子瓦片 mercator 直接枚举);RealTerrain 判定 `terrainSurfaceSourceForDraw` 提升具名(单一事实源)。
  - **⚠️ zoom 选择 = 相机相关(真机 debug 定案,踩 2 个 bug)**:① 不能用瓦片几何误差(静态 → 复现耦合 → capped z12 得 z12,真机首测 uniquePages=3/z12);② 用 tileSSE(`selectionFrameState.screenSpaceError`,距离已烘进去)但分母**必须地形细化阈值 16**(非 overlay MSE 2,用 2 深 3 级 → z18/435 页过取)→ `zoom=tileZ+round(log2(tileSSE/16))` = 匹配「未 cap 时会细化到的几何 LOD」= 耦合态清晰度。加 `Tileset::maximumScreenSpaceError()` getter。
  - **真机(Adreno,decouple+freeze+pageStore)**:近景 **uniquePages=125 zMax=z17 maxTileSse=674**(z17 匹配 #3 的 z16-17;dense 等价 3072 → **culls 96%** = 屏幕界定成立、非 tile-bounded 爆);地平线 171 页(partial heightmap → 仅 3 真实瓦片,余 proxy 走 mappedRaster)。**门② 机制验证通过**:zoom 相机正确 + 页集屏幕界定有界(hundreds,32MB 安全)。
  - **⏳ 遗留 ~1.8× 过取(近 125 vs #3 的 68)**:根因 thick-OBB 高度(z17 子瓦片 ~150m 宽,OBB 跨父瓦片全高度带 → 掠射假阳性);B2b 接池时收紧(per-subtile 高度或 screen-SSE 过滤)。
- **Step B2b ✅ 真机验收 PASS = 近景 z17 CRISP(commit `51a63e84d`,Adreno)**:稀疏 SVT 接线打通——池改页粒度(`blockLayers=1`,key=页(z,x,y),array=maxPages 512)+ PageEntry(pages_:layer/uploaded/fetchToken)+ TileIndir(per-tile gridN×gridN RGBA8 稀疏间接纹理,每帧按 resident 重建+sweep)。determination 从计数改驱动(逐 cell 视锥剔除→pool.acquire→新页 kickPageFetch→encode(layer,uploaded);miss→A=0);drainInbox 三重校验(页存在+layer 匹配+未 uploaded)+涓流。shader alphaOver factor 1.0→e.a(miss→保留 mappedRaster)。**真机:uniquePages=125 uploadedTotal=157 zMax=z17 glError=0,近景从 z14 糊 → z17 清晰**(楼宇/路网/车道线/球场/建筑阴影全锐,无缝无洞)。host 153/153+arm64 clean。
  - **⚠️ 根因坑(item①/B1/B2b 渲染验证一度全无效,重大教训)**:真实 heightmap DEM 表面**无 water-mask 元数据 → `useTerrainFormat=false` → 走 `kGltfFragmentGLSL`(非 `kTerrainFragmentGLSL`)**,而 SVT 页存储代码原只在 terrain shader。二者从未匹配 → pageStoreParams 设了但活跃 shader 无代码 → 静默渲染 mappedRaster。**z14-mappedRaster 与预期 z14/z17-pageStore 视觉难分,骗过截图**(机器信号页数/上传/绑定都真=误判来源)。3a 象限格实为 fill-proxy 瓦片(强制 `useTerrainVertexFormat=true`→terrainShader)。**修=页存储采样块搬进 kGltfFragmentGLSL+MSL**(sampler 10+2=12≤16 GLES 够/units 10/11 空;Metal texture 20/21 空)。**通用教训:改渲染必真机看像素+确认目标表面走哪个 shader,别信「糊得像」;shader 分裂时诊断法=全表面染纯色定位活跃 shader。**
  - ⏳ Metal 块已镜像加但无 Metal 设备验证;thick-OBB 过取(125 vs 68 ~1.8×)未收紧。
