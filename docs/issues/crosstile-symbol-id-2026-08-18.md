...
## 4b. 立项勘误(2026-08-18,开工前复核)

- **执行序 刀2→刀1**(原文刀1→刀2 反了):扩窗后才暴露两个正确性问题——
  ①同名多新实例双认领同一旧 entry(fade 互踩);②hit 路的锚点参考升级
  (`tileZ > e.zoom` 改写 entry 锚点)被双认领来回拉扯污染后续匹配。
  认领集在窄窗下落地=零行为变化纯基建,先行无险。
- **容差差距订正**:~10×(z14:我家 ±0.9m vs maplibre ±1 格 ≈ ±9.5m,
  格=1/256 瓦),非"两个数量级";粗代匹配细代时 maplibre 再乘 2^Δz。
- **误并风险订正**:相邻瓦同路名段中点相距半瓦级 ≫ 10m 窗,常态不误并;
  真实风险=密集短段下一旧 entry 被两新中点同时落窗(双认领,见上)。
- **认领集作用域**:maplibre 为 per-addBucket pass;我家 pass=一次瓦 commit
  (`buildTileSymbolGpu`)。per-commit 起步;跨 commit 语义(usedCrossTileIDs
  全貌)实现期对照源码再定,不预写。
- **窗宽标定**:4px 等效(1/256 瓦)为初值,验收时插 miss 距离直方图用
  自家数据校,不盲抄。
