...
- **重钳节流**:地形代次变化触发的 120 帧节流重镶原样适用(体高依赖采样高)。
  ...
  断线),重钳只为收紧体高。
- **dash**:终态 = **镶嵌期几何切分**(6c8f64569)。折线按里程切成一段段
  独立封闭墙带体、空隙不出几何,dash 边界即几何边界 → 零视差、像素级锐利,
  FS 只出纯色(顶点 24B,无 lengthSoFar)。
  - 选型依据(调研 `.ref/maplibre` + `.ref/cesiumjs`):maplibre line_sdf 的
    干净来自「光栅化的就是线 ribbon,linesofar 是顶点属性」——SDF 纹理与
    smoothstep 只是滤波,前提是里程坐标本身正确;cesium PolylineDashMaterial
    :22-25 干脆用 gl_FragCoord 旋转取相位绕开里程,代价是花纹锚在屏幕上、
    相机一动就在地面游动。我们光栅化的是体、着色的是地形像素,体面插值里程
    在侧视下有百米级视差——**坐标错误不是滤波错误**,故两条都不照搬。
  - 实现要点:切点沿段线性插值(划体恰是原连续墙带的子段,实线/虚线线形
    一致);划内保留原折点截面(否则划直穿弯道);闭环周期吸附成
    `L/round(L/period)` 使 seam 花纹连续;最短划 0.5m 与划数 4096 双护栏
    (超限降级实线)。
  - 中间态(已废弃,勿回退):FS 判里程 + 空隙 alpha=0 禁 discard + 底面先
    发射 + 闭环 seam 复制截面 + 色 pass 前置中心 ribbon——这些约束都是为了
    压住视差,几何切分后全部退役。
  - 遗留:划端头是硬几何边(与 fill 同为既定债,靠场景 FXAA);若日后 dash
    周期要接 zoom 表达式,需按整数 zoom 量化再走 120 帧节流重镶,否则连续
    缩放会每帧重镶。方案 A 回落 ribbon 的 `fract()` dash 仍是活路径。
