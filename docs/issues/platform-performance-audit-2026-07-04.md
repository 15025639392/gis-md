...
**修复进度（2026-07-04 晚）**：✅ P0-1/P0-2/P0-3/P0-4、P1-1/P1-2/P1-3/P1-9、P2-1/P2-3/P2-5/P2-8/P2-10/P2-11/P2-13/P2-14/P2-16/P2-17/P2-22 及 P2-20 的空 handler 部分已修复入库（commits 8d4d79e1f…ffcf1a141；P0-4 修于 2026-07-04 深夜）。真机 Adreno 730 实测：GLES submit 段 2.77ms→0.50ms（-82%，uniform 句柄化+VAO 合并效果，达成 §6 验收线）；Metal 完整 glTF PBR PSO 首次可创建；P0-4 常驻命令缓存后 render（命令构建）段稳态 5.3-7.5ms→4.3-7.0ms，截图像素级一致。未修：P2-4/18/19/21 及 P2-20 状态去重部分、P2-6 的 updatePriority 通道；P2-9 评审后降级缓办（专项）（P0/P1 已全部修复,P1-10 修于 2026-07-05 凌晨；P2-2/12/15 正确性组、P2-7、P2-6 保留槽修于 2026-07-05,macOS+Android A/B 均 0.000% diff；上采样 clip 主线程耗时已插桩实测,z13 单次 40-66ms,worker 化已立项）。P1-7/8(网络与解码链路)亦修于同晚:HttpCache shared_ptr 零拷贝+字节预算+put 移出网络线程;Android JNI 句柄缓存+线程生命周期 attach(修无条件 Detach 正确性隐患),真机 343 次解码零异常、截图与基线一致。P1-4/5/6(Metal 帧调度三件套)修于 2026-07-04 深夜:drawable 超时+in-flight 信号量+updateBuffer orphan+mipmap blit,MTL_DEBUG_LAYER 0 错误、140/140、macOS 截图与基线一致;顺带完成 P2-20 的"drawable 存 Impl 成员"部分。
...
- ✅[已修复 2026-07-05] **P1-10 上采样地形瓦片走主线程同步 prepare**（GltfRenderResourcePreparer.cpp:121-593；TilesetContentLifecycleCoordinator.h:197-202 显式回落 sync）：上采样子瓦片携带父级过期 terrainGpuVertexBytes，上传编排的尺寸校验不过→回落主线程同步 prepare（顶点全量重建+无预算 GPU 上传），叠加 P1-9 深缩放一帧可串多次。修复：clip 完成时（TileGltfTerrainUpsampledChildMaterializer）按 loader decode 同一公式为子瓦片重建预置字节→自然走 GpuUploadQueue 异步路（budget lane 节流），顺带消除"父级残留字节尺寸巧合匹配→上传父几何"隐患。真机 2000m 深缩放验证：zoom 15（全上采样），51 attachments 全 exact、0 missing/kicked、terrain pending 0、59.6 FPS；30000m 与基线像素级一致。注：clip 本身仍在主线程 TileLoadScheduler 里执行（审计设想的"工作线程 clip"并不存在）。**2026-07-05 插桩实测（Adreno 730，2000m 深缩放冷启动）：114 次 clip 合计 1460ms 全在主线程；z13 首跳（切 z12 父网格）单次 40-66ms（≈4 帧预算），z14 16ms、z15 5.5ms、z16 2.2ms（逐级 4× 递减）；启动期连发 22 次 40-66ms ≈ 主线程独占 1 秒。worker 化立项理由充分（需解决父模型生命周期共享），已挂专项任务；插桩日志 scope=Tileset.upsampleClip 保留。**RGB→RGBA 转码仅剩非地形 glTF 路径使用，不在地形热路径。

  **✅ clip worker 化已完成（2026-07-05）**：先跑 understand+design workflow（4 并行 reader 摸子系统 + 对抗式并发设计），设计**否决**了"裸指针 + hasContentLoadingUpsampledDirectChild reorder"方案（父重载 prepareGltfContent 仍析构旧 gltfModel → 悬垂，裸指针要堵 unload+reload 两条析构路）。**落地方案=主线程快照 + 显式 in-flight 认领**：createUpsampledModel 拆 buildClipInput(主线程读父模型/overlay 映射 + 整拷父 CPU 模型进 UpsampleClipInput 快照)/clipToModel(worker 只读快照跑裁剪),worker **零 TilesetTile/GltfModel 指针**→从根消除 UAF(与 P1-10 快照范式一致)。派发复用 requestContent 同构机制:新增 TileLoadRequestDispatcher::requestUpsampleClip(beginTerrainRequest 占 in-flight 名额+budget.tryIssue 门控 → AsyncSystem::pool worker → 完成回调单锁 enqueueCompletedLoadResult 投 pendingLoads),UpsampleClipCompletionGuard 保证 exactly-once(防 in-flight 泄漏死锁)。7 竞态场景(父 unload/reload、动画、并发 commit、双重投递、child evict、tileset 析构)逐一有防御。契约一/二公式不动。验证:141/141 native(含 2 新并发测试:去重 resubmit skip + 析构不死锁)；真机 Adreno 730 **主线程 clip 工作 ~1460ms → 18.9ms(仅快照拷贝,102 次 median 0.033ms/max 2.2ms),1908ms clip 全移 worker**；2km 深缩放 zoom16 **28 exact attachments、0 missing/kicked** 全收敛 58.8FPS；30km 重启与基线 0.000% 像素一致。残余:低空 selector=6.7ms 是独立的地形高度采样问题([[camera-terrain-height-perf]],未修)。
...
| ◐P2-6 | 优先级为提交时快照：Low 预取可占满 20 槽、High 无抢占、无 updatePriority 通道（cesium 每帧重排做不到） | CurlMultiRequestScheduler.cpp:295-306 | 部分修（2026-07-05）：High 保留 2 并发槽（min(2,max-1)），非 High 只能占 max-2，新视野 High 不再等完整轮转；updatePriority 通道（需动端口接口）未做 |
| ✅P2-7 | 主线程 pending 队列 O(N²) 优先级选取且持 lifecycle 锁扫描（积压数百时每帧数万次比较，阻塞工作线程投递） | TilePendingLoadQueue.cpp:145-192 | 已修（2026-07-05）：优先级 multimap（取头 O(1)、等键保插入序）+ cacheKey 哈希索引（contains/erase O(1)，dispatcher 的 containsCacheKey 线性扫描一并消除）；公共 API 不变，10+6+10+17 测试全绿 |
...
| P2-9 | 字符串瓦片注册表 key：每次 ensureTile 现场拼接（50 entries ≈ 每帧 500+ 次构造 / 2000+ 次分配） | TileCacheKey.h:10-13、TilesetTileRegistry.h:17 | 评审后降级缓办（2026-07-05）：字符串 cacheKey 贯穿 unloadQueue/lifecycle/requestState/pendingLoads 全链（tiles.find(cacheKey) 字符串查找 6+ 处），完整整数 key 迁移是 25+ 文件的管线级专项；P2-7 修复后残余成本仅每帧数百次小分配（个位数 µs），收益/风险比不支持顺手改。已挂专项任务 |
