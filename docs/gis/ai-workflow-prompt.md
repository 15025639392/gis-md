...
- 图形管线：Metal/OpenGL ES/Vulkan 能力（通过 RenderDevice 抽象）、坐标空间、shader（MSL/GLSL ES/SPIR-V）、材质、深度、透明、后处理、GPU 资源生命周期和性能指标
- 性能主线：数据约束、预处理、索引、LOD、缓存、worker、底层正确性门禁和压力场景
- 相关官方来源或项目约定
