...
| `tessellateTileMesh` / `appendTileSymbol` / `commitTileMesh` / `buildTileSymbolGpu` / `reclampTileBucketSymbols` / `bakeTileBucketLabels` / `dropTileMesh` | .cpp:1339-1372 / :1373-1403 / :1404-1459 / :1460-1505 / :1506-1585 / :1586-1627 / :1628-1631 | MVT 底图路径:瓦片即桶,镶嵌在 worker 完成(E1)。贴地时产 stencil 体(与 fill/line 流**互斥**);点要素 worker 出 TileSymbolCpu 实例表,commit 采地面高+图集解析定型 quad(符号刀A);带 name 实例存标签源,bakeTileBucketLabels 在字体就绪时补烘 glyph quads+placement 登记(符号刀B,幂等)。⚠️ 锚点高度是采样当刻的地形代次,地形细化会把它埋掉 → reclampTileBucketSymbols 在代次变化时按 tileSymbolSources 重钳(V24/B.6) |
| `buildRenderCommands` | .cpp:1632-1757 | 出命令总入口 |
| `visibleBucketKeys` | .cpp:1758-1827 | 可见桶筛选 |
| `updateLabelPlacement` | .cpp:1828-1901 | 标签避让 + fade + 地平线剔除(P5c) |
| `appendTerrainOcclusion` | .cpp:1947-1958 | 接地形深度 prepass 做符号遮挡(T2) |
| `appendBucketCommands` | .cpp:1959-2247 | 逐桶发命令:stencil 贴地面、贴地线、点符号/图标、标签 |
| `beginEditPreview` / `updateEditPreview` / `endEditPreview` | .cpp:2248-2263 / :2264-2270 / :2271-2296 | 编辑预览三接口(**编辑器本身不进引擎**,见该决策) |
| `pick` | .cpp:2340-2558 | 要素拾取 |
