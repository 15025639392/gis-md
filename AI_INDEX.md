...
| `TileEdgeMismatchProbe` (224) | `measure` (TileEdgeMismatchProbe.h:165)、`renderedHeight` (TileEdgeMismatchProbe.h:143)、`sourceOf` (TileEdgeMismatchProbe.h:122)、`edgePoint` (TileEdgeMismatchProbe.h:156) | **跨瓦接边几何错位**直接测量(①-1 判据)。漏天像素已恒 0 但 ε 被裙墙盖住,故换尺子。⚠️ 两侧必须走 `sampleHeightRenderGrid` 单一事实源;⚠️ 必须乘 `terrainReliefFade`,漏了会读出假错位;⚠️ fadeUniform/fadeDiffer **必须分开统计**,后者量级大一个数量级会把 ①-1 的效果稀释掉 |
