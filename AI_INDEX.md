...
Engine  (Engine.h/.cpp — thin lifecycle + surface + input facade)
  ...
  └── Scene  (scene/Scene.h/.cpp — owns Camera, CameraSystem, Renderer, SceneRenderPipeline)
        ...
        └── Tileset(s)  (tiling/Tileset.h/.cpp — cesium-native Tileset equivalent)
              ...
              ├── RasterOverlay mapping       → ActivatedRasterOverlay → DirectRasterMapping,
...
| `rayIntersectionInterval` | .cpp:203-261 | Unit-sphere quadratic in scaled space; q²>1/<1/=1 branches; cesium `rayEllipsoid` semantics (entry/exit params) |
...
| Item | Lines | Algorithm |
|---|---|---|
| `fromEarthCenteredEarthFixedCoordinates` | .cpp:23-43 | Scales both endpoints to geocentric surface; either fails → `nullopt` |
| `fromLongitudeLatitudeHeight` | .cpp:44-53 | Cartographic→ECEF then ECEF factory |
...
| `translation/scale/rotationX/Y/Z` | .cpp:32-32 | via `glm::translate/scale/rotate` |
...
| `rayEllipsoid` | .cpp:78-138 | scales ray by inverse radii, quadratic in unit-sphere space; returns `RayEllipsoidIntersectionInterval` |
...
| `update(FrameState, IPrepareRendererResources*)` | .h:106-107 / .cpp:499-516 | Per-frame entry; delegates to `TilesetUpdateFrameFacade::update`; logs if >5ms |
| `buildRenderCommands(Renderer&, RenderCommandList&)` | .h:108 / .cpp:518-556 | `++frameNumber_`, `renderCommands_.beginFrame(...)`, then `TilesetRenderFrameExecutor::buildRenderCommands` over `tilePlan_` |
| `releaseRenderReferences()` | .h:125 / .cpp:664-670 | Called by Scene after `renderer_->submit()`; drops the ref added in buildRenderCommands via `TileRenderReferenceReleaser::release` |
...
Drives one `update()` frame; runs as friend of `Tileset`. `run(tileset, frameState, pPrepRenderer)` (.h:17-21 / .cpp:134-399).

| Step | Lines | Action |
| --- | --- | --- |
| generation bump | .cpp:104 | `++tileset.generation_` each frame so RenderCommand validator accepts commands |
| frame work | .cpp:106-208 | `TileFrameWorkCoordinator::run(...)` with lambdas: processPendingLoads, markContentResourcesDirty, hasPendingWork, `TileRenderPlanFrameRefresher::refresh`, `TilesetSelectionFrameFacade::selectTiles`, ensureTile, unloadTileContent, requestMissingContent |
| drain GPU uploads | .cpp:212-222 | `tileset.drainGpuUploadQueue(pPrepRenderer)` after selection; logs if >1ms (Android) |
| result | .cpp:245-333 | Packs `TileUpdateDebugLogInput` (visible count, load queue size, compute/prefetch/request/upload ms, reuse mode/reason) |

**`kPostInteractionResourceSmoothingSeconds`** = 1.25 (.cpp:21).
...
| Method | Lines | Algorithm |
| --- | --- | --- |
| `initialRefineDecision` | .cpp:6-14 | `refine = unconditionallyRefine \|\| (!meetsSse && !ancestorMeetsSse)` |
...
| `applyOcclusionAction` | .cpp:48-60 | stop → `refine=false, meetsSse=true` |
| `continueDeeperDecision` | .cpp:61-78 | non-renderable refined ancestor keeps descending; `queueUrgent = !ancestorMeetsSse` |
...
cesium-native `OctreeTilingScheme` equivalent for 3D voxel/implicit octrees. `OctreeTileID{level,x,y,z}` (.h:10-21). `AxisAlignedBox` + `rootTilesX/Y/Z`. `tileCountX/Y/Z(level) = rootTiles << level` (.cpp:14-24). `positionToTile(Vec3,level)` (.cpp:58-79); `tileToBox(OctreeTileID)` (.cpp:58-77) — note it builds the box from origin-relative sizes (min = size·index).
...
| `TileRenderCommandManager` (61) | ctor (.cpp:33-67)、`beginFrame` (TileRenderCommandManager.cpp:20)、`buildTileDrawCommand` (TileRenderCommandManager.cpp:30) | 常驻命令缓存。⚠️ **勿把逐帧字段写回常驻命令** |
| `TileRenderPlanFrameRefresher` (280) | `refreshFrameCredits` (TileRenderPlanFrameRefresher.cpp:178);`collectReadyRasterTileCredits` (TileRenderPlanFrameRefresher.cpp:134)、`collectRenderContentCredits` (TileRenderPlanFrameRefresher.cpp:153)、`collectRasterOverlayProviderCredits` (TileRenderPlanFrameRefresher.cpp:160)、`collectDirectRasterProgress` (TileRenderPlanFrameRefresher.cpp:199)、`logEdgeMismatch` (TileRenderPlanFrameRefresher.cpp:66)、`formatEdgeStats` (TileRenderPlanFrameRefresher.cpp:40) | 归属信息与栅格进度;接边错位探针的**逐帧累积 + 周期报告**(单帧 n≈17 撑不起 A/B)。⚠️ **H-S5(2026-08-21)**:新瓦首建预算 4/帧(交互)/8/帧(恢复),耗尽且祖先可渲染时改走祖先裁剪回退(`renderEntryFirstBuildDeferredCount`),摊平扫掠前沿首建 burst |
...
| `takeHighestPriorityUpload` | .cpp:163-200 | Skips non-Urgent when `interactionActive`; gated by `tryFinalize` on `ContentFinalize`/`TerrainFinalize` lane (`uploadLaneForDomain`, .cpp:142-164). |
...
| `finishRenderResourcePreparation` | .cpp:73-88 | On failure: restore initial bounding volumes, release overlays, `markGltfRenderResourcesFailed`. Returns `{resourcesReady, true}`. |
...
| `markEligibleForUnloading` / `markIneligibleForUnloading` | .cpp:70-82 | Push/remove from `unloadQueue_` via `TileIndexState`. |
...
| `update()` | .h:89-100 / .cpp:156-504 | 7-step flow; params: geometryKey, overlayDetails, targetScreenPixels x/y, provider, prepRenderer, missingProjections, parentTile, overlayIndex, hasRenderContentDetails, boundingVolumeRectangle |
| — stale-handle drop | .cpp:173-198 | If ready/loading tile no longer owned by provider (option/coverage change) → releaseTileReferences or drop loading handle + recompute state |
| — Step 1 attached fast path | .cpp:200-221 | If `Attached`: revert to Unattached if renderer resources gone; else markUsed + return MoreDetail from `_pReadyTile->isMoreDetailAvailable()` |
| — Step 2 failure fallback | .cpp:223-259 | Inside `update()`, on `overlayTileCannotRenderItself` (loading-tile `Failed` **or** empty composition), set `originalFailed_`, walk parent geometry chain via `findParentTileOverlayPreferLoading`, adopt ancestor's loading/ready tile |
...
| — placeholder retry | .cpp:285-295 | Placeholder loading tile cleared once `provider.isReady()` |
| — map-to-geometry | .cpp:297-383 | If no loading tile: provider-not-ready→placeholder (texCoordID=-1); render-content+geometryRectangle→`provider.mapRasterTilesToGeometryTile`; missing projection→record in `missingProjections` at index-after-list + placeholder; else boundingVolume path |
| — Step 3 loading→ready promote | .cpp:391-428 | Inside `update()`, on `Loaded`/`Done`: detach old ready, `_pReadyTile=_pLoadingTile`, cache texture, `computeTranslationAndScale`. Gated by `canPromoteReadyRaster` (needs prepRenderer OR Unattached OR no ready) |
| — Step 4 ancestor substitution | .cpp:76-100 / :430-467 | While loading, walk parent chain via `findLoadedTileOverlay` for a ready ancestor tile; adopt as `_pReadyTile` (source=Ancestor), recompute UV for CURRENT child bounds. Empty-composition candidates are **skipped** so the walk reaches a drawable ancestor instead of stopping at a blank one |
| `overlayTileCannotRenderItself()` | .cpp:71-74 | `Failed` **or** `RasterOverlayTile::isEmptyComposition()` — the two endings that must fall back to the parent chain, else the tile and its whole descendant chain render blank |
| — Step 5 failed-no-fallback valve | .cpp:469-482 | Failed loading tile + no ancestor + no ready → mark ready (texture null) `Attached` so raster failure never blocks geometry |
| — Step 6 attach | .cpp:485-487 / :626-650 | `_pReadyTile->loadInMainThread()`; if prepRenderer + renderer resources → `attachRasterInMainThread(geometryKey, overlaySlot, tile, tex, offset/scaleUV)`; state=Temporarily/Attached |
| — Step 7 return MoreDetail | .cpp:489-503 | loading→Unknown; else MoreDetail from ready tile unless `originalFailed_` |
| `isMoreDetailAvailable()` | .cpp:506-514 | `!loading && !originalFailed && ready && ready.moreDetail==Yes` |
| `hasPendingNonPlaceholderLoadingTile()` | .cpp:516-520 | Loading tile present and not Placeholder |
| `detachFromTile()` | .cpp:605-618 | `detachRasterInMainThread(geometryKey, overlaySlot)` (skips Failed tiles / z<0), state→Unattached |
| `releaseTileReferences()` / `clearTileOwnershipState()` | .cpp:620-624 / :652-670 | Detach + drop both handles, reset UV/flags/texCoordID/slot |
| `loadThrottled()` | .cpp:672-684 | markUsed + `provider.loadTileThrottled(loadingTile, budget)`; no-op for null/placeholder |
| `computeTranslationAndScale()` | .h:122-124 / .cpp:686-710 | Delegates to `TileSurface::computeTranslationAndScale`; non-inverted V passes through `textureWindowForNorthWestUv`. Pure rectangle-ratio, no tile-size assumption (edge bleed handled by GL CLAMP_TO_EDGE) |
...
Free helpers (.cpp:17-26): `addProjectionToList` (dedup append), `findLoadedTileOverlay` (ancestor ready lookup, same-owner check via `hasSameOverlayOwner`), `findParentTileOverlayPreferLoading`, `findRectangleForProjection`, `isCurrentProviderTile`.

### RasterOverlayRuntime.h / .cpp

Tileset-owned migration boundary for the ordered raster overlay stack. It owns the sole configured overlay order, freezes a slot-preserving frame view, owns independent Direct/PageStore generations, normalizes backend provider subsets back to SDK order, and exposes the shared decoded-source access point through that frame view. The former Tileset overlay alias and pre-frame provider/depot convenience APIs are removed.

| Item | Lines | Description |
| --- | --- | --- |
| `RasterOverlayFrameSlot` / `RasterOverlayFrameContext` | `RasterOverlayRuntime.h` | Stable `runtimeSlot`; Direct exclusions remain null slots; frame/generation/provider/depot views are published together. `visible/opacity/role/priority/fallbackPolicy/blocksCompleteRenderable/projection/providerRevision/generation` are value-snapshotted for the frame. |
| `configuredOverlays` | .h:104-107 | Stable Runtime-owned configuration stack for Scene convergence, diagnostics and cache accounting; it is distinct from a backend's frame view. |
| `providersForBackend` | .cpp:133-179 | Private publication helper: calls the selected provider-view strategy, removes foreign/duplicate providers, and restores Runtime order without instantiating excluded overlays. Execution consumers cannot use it to bypass the frame snapshot. |
| `beginFrame` | .cpp:181-330 | Publishes both backend views/generations. Initial generation does not invalidate pre-existing Direct work; later Direct changes release mapping/direct state before the new view is consumed. |
| `setBackend` / `setBackendEnabled` | .cpp:332-352 / :361-374 | Replaces or toggles one phase-one provider-view strategy; changes become visible at the next `beginFrame`. This is not yet a full selection/request/upload/render executor contract. |

### RasterResolution.h / .cpp + RasterBindingSet.h / .cpp

Backend-neutral semantic resolution plus backend-specific Direct sampling. `RasterResolution` orthogonally records request state, exact/composed content, own/ancestor relation, attachment, coverage/readiness, target pending/failure, desired/resolved zoom, presentation policy and frame generation. It deliberately carries no texture/UV. `DirectRasterSampleDescriptor` carries only Direct `Texture*`, texcoord, UV affine and erased resource lease; PageStore keeps its array+indirection+placement descriptor shape. `RasterBindingSet` publishes both by Runtime slot, and draw/readiness no longer read `DirectRasterMapping*` through their downstream binding contract.

| Item | Description |
|---|---|
| `resolveDirectRasterBinding` | Adapter from Direct mapping lifecycle to `RasterResolution + DirectRasterSampleDescriptor`; preserves failed/empty target while an ancestor remains drawable, and keeps desired source zoom independent from displayed ancestor zoom. |
| `rasterResolutionAllowedByPolicy` | Backend-neutral visibility/opacity/role/fallback gate; `SkipUntilReady` is expressed by `targetPending`, not by rereading Direct loading pointers. |
| `RasterBindingSet::resolve(..., frame)` | Consumes frozen slot metadata/generation; a filtered Direct slot remains non-visible/non-drawable even though its configured overlay pointer still exists. |

### SurfaceRasterBinding.h / .cpp

Decides whether a Direct raster mapping is drawable and produces the render-time binding (texture + UV transform). "Drawable" = ready tile is a real Loaded/Done raster with an owned texture.
...
| `isLegalSurfaceRasterTile()` | .cpp:8-16 | Tile non-null, has texture, state Loaded/Done |
...
Per-tile collection of `DirectRasterMapping` mappings (one slot per active overlay), plus missing-projection accumulation and mapping-identity invalidation. Lives on `TilesetTile.rasterOverlayState`.
...
| `synchronizeMappingIdentity()` | .cpp:99-108 | On identity change, `releaseAndClearReferences` then store new identity |
...
| `forEachMapping()` | .h:51-56 | Visits each slot (nullable) — used by ancestor lookups in DirectRasterMapping |

### TileRasterOverlayPrefetcher.h / .cpp

Per-tile prefetch driver: syncs mapping slots to the active-overlay list, runs `DirectRasterMapping::update` for each overlay in priority order, then throttled-loads the resulting loading tiles. Selection-phase (no renderer resources; `pPrepRenderer` optional).
...
| — per-overlay loop | .cpp:52-139 | For each `overlayProcessingOrder` index: skip invisible (release slot), `ensureTileProvider(device)`, compute geometry/boundingVolume/target rectangles, `computeDesiredScreenPixels`, then `DirectRasterMapping::update(...)` |
...
| Method | Lines | Description |
| --- | --- | --- |
| `doneTileCannotHoldRasterOverlays()` | .cpp:18-23 | True if tile `Done` but not Render content OR `!hasRasterOverlayHostContent()` |
| `requiredOverlaysReady()` | .cpp:25-43 | For every visible overlay with `blocksCompleteRenderable()`, require `hasReadyMapping(i)` |
| `processingOrder()` | .cpp:167-186 | Stable-sort indices by overlay `priority()` descending (null → `RasterOverlayPriority::Low`) |
...
| `projectEffectiveContentBoundingVolumeRectangle()` | .cpp:293-308 | Projects tile's content/bounding volume Region rectangle (nullopt if not Region kind) |
...
Header-only. Resolves which rectangle/projection context a tile presents to `DirectRasterMapping::update` — render-content details vs. bounding-volume fallback.
...
| `computeDesiredScreenPixels(bounds, geomErr, mse)` | .cpp:88-107 | `diameterMeters × mse / geometricError`, clamped ≥1; geometricError≤0 → default |
...
| Method | Lines | Description |
| --- | --- | --- |
| `toTextureFilter` / `toTextureWrap` | .cpp:111-125 | glTF sampler enums → `TextureDesc` enums. |
...
Emits per-primitive `RenderCommand`s from prepared `GltfPrimitiveRenderResources`, binding materials, textures, water mask, and Direct raster overlay samples. cesium-native draw-call assembly.
...
| Method | Lines | Description |
| --- | --- | --- |
| `sampleHeight` | .cpp:84-112 | `u=(lon-west)/width`, `v=(north-lat)/height`; rejects (returns 0) if outside `[0,1]±ε` else clamps; **`kTileCoordinateEpsilon`** = 1e-12 (.cpp:84-112); `sampleBilinear` then `isNoData`→0. |
...
| `SingleGltfContentProvider` | .h:179-231; .cpp:3740-3754 | Single glTF/GLB/B3DM tile. `decodeContent` (.cpp:3587-3594) → `GltfParser::parse`; `setEastNorthUpPlacementDegrees` (.cpp:3754) places model via ENU frame. |
| `TilesetJsonContentProvider` | .h:237-312; .cpp:4021-4038 | 3D Tiles `tileset.json`: `parseTilesetJson` (.cpp:4038) parses transforms/refine/GE/bounding volumes/URIs incl. external tilesets; `decodeRenderableContent` (.cpp:4020) parses glTF w/ up-axis transform. cesium-native `TilesetJsonLoader`. |
...
| Item | Lines | Description |
| --- | --- | --- |
| `createScheme` / `rootRectangleForScheme` / `quadtreeChildrenForKey` (anon) | .cpp:35-40 / :33-37 / :39-48 | 匿名命名空间的分片方案辅助。 |
| ctor | .cpp:53-59 | `schemeId="XYZ-WebMercator"`, `maximumLevel=14`, `gridSize=16`。 |
| `id` / `supportsTile` / `rootTiles` | .cpp:77-81 / :65-73 / :75-77 | Provider 身份与覆盖判定。 |
| `tileMetadata` / `childTiles` | .cpp:113-125 / :111-122 | 四叉树导航。 |
| `availabilityState` | .cpp:126-131 | 只看 `key.z`(**非空间性** —— 这正是 `GltfTerrainUpsampler` 的 TerrainAvailability 触发路运行时不可达的原因,见该文件头注释)。 |
| `requestTileContent` (2 个重载) | .cpp:132-144 / :143-193 | 合成内容,无网络。 |
...
| Item | Lines | Description |
| --- | --- | --- |
| `splitHighLow` / `setLocalPosition` (anon) | .cpp:45-51 / :38-43 | ECEF 高低位拆分(`kSplit`=65536.0)写入 `SurfaceVertex.position`。 |
| `projectRasterRectangle` (anon) | .cpp:52-58 | 地理矩形 → 目标投影矩形。 |
| `rewriteProjectionTexCoords` (anon) | .cpp:59-102 | 把 UV0 重投影进 WebMercator/Geographic 栅格空间。 |
...
### RasterAssetDepot.h / .cpp

Runtime-owned consumer-neutral facade for decoded raster assets. `acquireExactSource` (.cpp:7-30) and `tryGetCachedExactSource` (.cpp:32-45) route Direct/PageStore to the same provider source depot and wrap returned images in accounted external leases; `stats` / `record` (.cpp:47-75) expose cache-hit, joined-inflight, started-transport and admission-denied counters by consumer. Physical cache/inflight ownership remains provider-local, so this is a shared access/de-duplication boundary rather than a cross-provider global asset hash.
...
| `id()` | .cpp:251-256 | `"xyz-" + hash(urlTemplate)` |
| `setOpenGlobusGroupedY` | .cpp:271-277 | Enables 3-band grouped-Y (mercator/north/south), forces schemeId `"OpenGlobus-Earth"` |
...
| `requestDiagnostics` | .cpp:675-689 | Atomics started/completed + transport max active |
| `decodeTile` | .cpp:690-717 | Prefers `PlatformBridge::decodeImage`, else `stbi_load_from_memory(...,4)` → RGBA |
| Android failure log cap | .cpp:41-41 | **`kMaxAndroidFailureLogs`** = 24 |
...
| availability | .cpp:769-774, 631-762 | Viewport→tile ranges, `addAvailableTileRanges`, `applyViewportAvailability`, `isTileKnownAvailable`, `isTileInCompleteAvailabilityRange` |
...
| `buildUrl` | .cpp:66-80 | Simple `{z}/{x}/{y}` string replace |
...
| `loadInMainThread` | .cpp:64-77 | Loaded→Done transition (cesium-native GPU-resource step) |
| `getTargetScreenPixelsX/Y` | .h:80-85 | cesium-native `getTargetScreenPixels` (default 256×256) |
| direct-composite fields | .h:137-174 | Mapped (non-source) tile: source zoom/bounds/keys + min/max XY of source-tile plan |
...
### RasterOverlayTileProvider.h / .cpp + RasterOverlaySourcePlan.h / .cpp + RasterOverlayImageCompositing.h / .cpp + RasterOverlayTileProviderBudget.cpp + RasterOverlayTileLoadDispatch.cpp + RasterOverlayUploadProcessor.cpp
...
| Method | Lines | Description |
|---|---|---|
| ctor / dtor | .h:51-55 / .cpp:632-644 | Takes `ImageryProvider&`, `TileScheme&`, optional uploader (null = headless test); dtor drains async state |
| `getTile` | .cpp:1161-1195 | Get/create cached tile by key; returns shared placeholder when not ready; stamps `frameNumber_` |
| `mapRasterTilesToGeometryTile` | .cpp:1196-1303 | cesium-native equivalent: geometry rect → quadtree source plan; exact single-source → direct tile, otherwise build a Direct composite tile |
| `buildRasterOverlaySourcePlan` | `RasterOverlaySourcePlan.cpp` | Backend-neutral immutable geometry→imagery plan: MSE zoom, maximum-texture downgrade, antimeridian coverage, Cesium 1/512 edge trim, source keys/range and exact-single-source classification. Direct resolution and composite loading share this function. |
| `resolveTile` | .cpp:1304-1329 | Best available tile ≤ desiredZoom over bounds; nullptr if none |
| `loadTile` / `loadTileThrottled` | .cpp:1402-1470 | Start async load (Loading + HTTP); throttled by `maximumSimultaneousTileLoads` (=20, .h:189) |
| `composeQuadtreeSourceImagesWithDetails` | .cpp:588-623 | Composite source images into target rect; propagate MoreDetailAvailable/credits/diagnostics |
| `projectedVForLatitude` | .cpp:624-631 | Latitude → projected V within bounds |
| `trimUnusedTiles` | .cpp:1570-1630 | Evict tiles by `lastUsedFrame`; advances `frameNumber_` |
| `refreshSourceAssetDepot` | .cpp:865-873 | Rebuild shared source-tile depot on option change |
| `requestDiagnostics` | .cpp:1330-1356 | Aggregates imagery-provider + raster-source request counters |
| 上传处理簇 | **RasterOverlayUploadProcessor.cpp** | I-P4 拆刀 5:主线程上传推进——`processPendingUploads` .cpp:57(逐瓦片 pendingUploads 消费,`uploadAllowedDuringInteraction` 交互期尺寸过滤 .cpp:33)、`hasPendingWork` .cpp:393;专属 3 常量 + 1 helper,零主文件残留依赖 |
| 加载/派发簇 | **RasterOverlayTileLoadDispatch.cpp** | I-P4 第四刀 4b:从主文件拆出的 load/issue 核心——`loadDirectCompositeTile`/`pumpLoadingDirectCompositeTile` .cpp:96/194、`loadSourceTileList`/`loadSourceImageSet` .cpp:217/234、`issueDirectCompositeSourceImageSet` .cpp:475、`estimateNewSourceRequestsForSourceKeys` .cpp:548、`issuePendingSourceFallbacks` .cpp:580、`issueActiveDirectCompositeSourceImageSets` .cpp:687;自带 5 个专属匿名 helper(日志/请求槽/在飞容量/waiter token/瞬态失败),复用收口后的共享几何集 |
| 投影+合成簇 | **RasterOverlayImageCompositing.cpp** | I-P4 拆出:投影工具(`projectionForSourceScheme`/`projectionForScheme`/`webMercatorY`/`projectedSouth/North/Height` 等 .cpp:298-381)+ 几何覆盖映射(4a 收口:`inwardSampleEpsilon` .cpp:384、`mapGeometryBoundsToImageryCoverage` .cpp:427、`effectiveCoverageRectangle` .cpp:506 等)+ 像素合成(`unsafeBlitImage`/`unsafeBilinearResize` .cpp:186-296、`blitImage` .cpp:519、`combineQuadtreeSourceImages` .cpp:615、`composeDirectCompositeSourceImageSet` .cpp:744)+ `RasterSourceResult`(h)+ 通用判断(`isDecodedImageUploadable`/`decodedImageSizeBytes` 等) |
| source plan 合同 | **RasterOverlaySourcePlan.h / .cpp** | directComposite 退场首刀：纯值 `RasterOverlaySourcePlan` + coverage/range，零 cache/request/upload/attachment/GPU 指针依赖；保留 Cesium MSE、边界容差、反经线与 unsupported-child→source-parent fallback 的枚举语义。 |
| depot 嵌套 struct | **RasterOverlaySourceDepot.h** | I-P4 第二刀:QuadtreeSourceAssetDepot(源瓦片共享缓存/in-flight/条件请求,约 670 行)+ DirectCompositeSourceImageSet(映射源集合派发/合成回调,约 290 行);依赖 ProviderAsyncState 私有状态,嵌套类定义在头文件使主文件成员访问可见 |
| 状态预算簿记 | **RasterOverlayTileProviderBudget.cpp** | I-P4 第三刀:静态成员函数,pending-upload 字节记账/backpressure(`pendingUploadSizeBytes`/`retain|releasePendingUploadImageBytesLocked` 等)+ source-depot 预算/LRU(`enforceSourceDepotBudgetLocked`/`compactSourceDepotCacheLruLocked`)+ 失效清空(`clearSourceDepotCacheLocked`/`clearPendingUploadsLocked`);只依赖头文件完整类型与 `RasterOverlayImageCompositing.h` 共享自由函数,零匿名 helper |
...
| `RasterOverlaySourcePlan` / `RasterTileMapping` | `RasterOverlaySourcePlan.h` / provider .h | Backend-neutral source plan (bounds, keys, min/max XY, exact-single-source) and temporary provider mapping result (`directTile/sourceTiles` names retained for compatibility in this slice). |
| `ProviderAsyncState` | .h:394-447 | Shared state that **outlives the provider** for in-flight callbacks (cesium-native depot lifetime): `pendingUploads`, source depot cache/in-flight/LRU, active Direct composite source sets, atomics, `revision`, async-destruction promise |
...
| `acquire` | .cpp:73-238 | 取(或建)一份模板几何;命中比对建模方 schemeId(T-P5 看门) |
| `heightCacheKey` | .cpp:261-273 | 高度纹理缓存键 |
| `findHeightArray` / `ensureHeightArray` | .cpp:274-279 / :280-312 | 按 gridSize 取/建高度 texture array |
| `bakeTerrainHeightNormalTexels` | .cpp:313-448 | 烘高度 + **切空间法线到 B/A 通道** |
| `acquireHeightTexture` | .cpp:449-568 | 取高度纹理层。⚠️ 末尾 4 行是边 LUT(①-1),acquire 时必须初始化成「差值 0」——层是 LRU 复用的,且**全零字节不是差值 0**(0 落在 ±2048m 量程中点 q=32768)。B 方案开启时(GLES only)不在此做 CPU 烘焙,改把源打包 push 进 `pendingBakes_` 延后 GPU 烘 |
| `ensureBakeResources` | .cpp:569-585 | B 方案 GPU 烘焙的懒初始化:建烘焙 shader(`TerrainHeightBakeShader.h`)+ 全屏 quad;首次 `flushHeightBakes` 前调用 |
| `flushHeightBakes` | .cpp:586-673 | B 方案:每帧 `SceneRenderPipeline` 在 `buildLayerCommands` 后调用,把 `pendingBakes_` 逐个 RTT 烘进 height/normal texture2DArray 层(externalColorTarget + setFramebufferColorLayer)。burst(>1)时打 `HeightBakeFlush` 诊断(2026-08-21 GPU swap 尖刺排查;真机已排除烘焙为尖刺源)。仅 GLES 走此路,Metal/Vulkan 由后端守卫回退 CPU |
| `updateEdgeLutRows` | .cpp:674-701 | 写本帧边吸附的邻居高度差表(①-1)。**H-B1+H-S4(2026-08-21):字节 diff 相同跳过;变化者只入池(不再立即上传),帧末 `flushEdgeLutUploads` 批量灌入**—— 运动期 frameState 6.8-14.6ms 的逐层小上传被摊成每 array 一次调用。瓦片不在该档驻留时返回 false,调用方据此清 lutValid 位 |
| `enqueueEdgeLutUpload` / `flushEdgeLutUploads` | .cpp:702-718 / :719-814 | H-S4 批量上传:本帧待传层按 array 拼成连续内存(中间未变层用缓存/差值 0 填充),每 array 一次 `updateTextureArrayRegion`(GLES depth=层数单次 PBO)。失败保留旧缓存,下一帧字节 diff 自动重试 |
| `touchHeightTexture` | .cpp:819-828 | LRU 触碰 |
...
| 项 | 行 | 说明 |
|---|---|---|
| `createScheme` / `rootRectangleForScheme` / `quadtreeChildrenForKey` (anon) | .cpp:40-45 / :39-43 / :45-58 | 分片方案辅助 |
| `makeHeightSampler` | .cpp:61-93 | `EllipsoidProxyHeightSampler`(相机高度查询用) |
| ctor | .cpp:93-102 | |
| `id` / `supportsTile` / `rootTiles` | .cpp:115-119 / :104-112 / :114-117 | |
| `tileMetadata` / `childTiles` | .cpp:151-163 / :150-161 | |
| `availabilityState` | .cpp:164-182 | ⚠️ **只看 `key.z`** —— 非空间性(见 TileAvailability 一节的说明) |
...
| `CameraGroundState` | .h:144 | solver `GroundState` 的别名转发 |
| `constraintSolver_` / `manipulator_` | .h:191 / :193 | 二者都由本类拥有;操控器持 `camera_` 与 `&constraintSolver_` 的裸指针,**不回指本类** |
...
**为什么独立成类**:后面还会有语义完全不同的第二、第三个实现——Tethered 模式
(拖拽=绕载体转,不是绕地心转)与桌面输入(滚轮/中键/方向键,事件形状不同但共用
锚点数学)。焊在一个类里就只能靠 `if (mode == ...)` 把两套数学缝进同一个函数体。

核心不变量:单指拖拽先抓地表点、移动时让该点跟手;双指手势中**只有锚点钉合
(pin)产生横向世界运动**,dolly/twist/pitch 在数学上全部严格保锚,与 pan 正交
——没有意图分类。

| 项 | 行 | 说明 |
|---|---|---|
| `SurfacePicker` typedef | .h:40 | Scene 注入的地形拾取;未注入/未命中时回退内部 WGS84 球面拾取 |
| `PinchMode` | .h:55 | `Undecided`/`Manipulate`/`Pitch`。latch 在 InputManager,一段手势最多迁移一次 |
| `PinchInput` | .h:62 | **绝对量表述**(当前 spread/起手 spread、连线角累计):事件被合并或丢弃时不产生累积漂移 |
...
| 方法 | 行 | 算法 |
|---|---|---|
| `anchorExactWeight` (anon) | .cpp:80 | 病态区混合权重:入射余弦 c≥0.35 全精确,≤0.10 全转台,中间 smoothstep。精确解增益 ∝1/c 掠射时爆炸,而硬切换必然跳变或死锁——连续混合 + 退化区整点重取锚点是唯一同时消掉两者的做法 |
| `onDragStart` |  .cpp:127  | `grabSurfacePoint`;清零惯性;起手判定模式(契约 1.2,对齐 Cesium spin3D:海拔<150km 且起手射线与地表掠射 |dot|<0.05 → strafe 近地拖图,否则空间拖球) |
| `resolveDragMode` | .cpp:279 | 单指模式判据(2026-08-22 对齐 Cesium spin3D):WGS84 海拔 <150km 且起手拾取射线与地表**掠射**(|dot(rayDir, 法线)|<0.05,约地平线 3° 内)或相机在拾取点内侧 → strafe;否则空间拖球。不再用 pitch≥60° 硬阈值 |
| `applyNearGroundPan` |  .cpp:307  | 近地拖图(契约 1.3,2026-08-22 对齐 Cesium strafe):平面 = 过锚点、法线 = **相机视线方向**(视线法平面在地平线处不退化)⇒ **无地平线钳制**,地图自由跟手;平移量=锚点−射线∩平面;收集像素速度样本 |
| `tickNearGroundInertia` |  .cpp:361  | 近地惯性(契约 1.4):方向锁定、v*=0.998/ms 衰减、折合屏幕位移 <0.5px/帧即停,**无地平线边界**(对齐 Cesium 惯性)。**2026-08-19 起速度样本采原始手指位移**;每步经 `repinNearGroundAnchor` 重钉保锚 |
| `repinNearGroundAnchor` | .cpp:422 | **2026-08-20 新增,爬升保锚(C-V1 扩展)**:近地掠射下 clampNow 保锚 dolly 因 eye→anchor 近水平退径向抬升,锚点像素被抬离等效手指(真机 anchorErr 峰 374px)。用抬升后相机重投影(平移量=锚点−射线∩平面)把锚点补回指下,再 clamp 收残差;每帧爬升预算由 constrainEye 共享(25m/帧),循环不叠加弹跳,最多 2 轮 |
| `onKeyCommand` / `panByPixels` / `zoomByLevels` |  .cpp:468  / :512 / :545 | 键盘(契约 3.3):方向键平移 100px(空间=转台,近地=strafe,中心射线 miss 时取下半屏地表)、+/- 缩放 ±1 级(Shift 加倍)、Shift+方向键旋转 15°/倾斜 10°;入口先 `clearGlideInertia`;**仅 Free 模式生效**(CameraSystem 侧先查 active 控制器) |
| `onPinchGesture`(新契约) |  .cpp:601  | 契约 2.2 组合:①dolly 仅 `zoomEngaged` 时(阈值 0.1 log2;**2026-08-22 对齐 Cesium zoom3D:缩放锚点 = 屏幕中心拾取点**,WGS84 椭球拾取,非手指质心;滚轮 `smoothZoom` 只并入目标不瞬时施加) → ②twist 仅 `rotateEngaged` 时(25px 弧长,绕锚点法线) → ③Pitch 锁启用时绕锚点竖转(反 wind-up:被守卫拒绝时重取基线) → ④`applyPinchPin`(**唯一产生横向世界运动的通道**;Undecided 钉起手质心,激活后钉当前质心=平移随动)。**2026-08-22:无锚回退**——质心在天空/掠射区锚点被拒时,Pitch 轴退化为绕相机自身竖转(eye 不动),双指倾斜不失效。**2026-08-23:无锚 twist 回退**——twist 是正向旋转(角度直接来自手指,不经过 pin 反解),掠射/远锚守卫只保护 pan,不误杀旋转;**绕指下地表点竖直轴旋转**(地图旋转语义,奥维同款):双指中心的地表点锚定并钉在指下、地图画面绕它旋转、地平线保持水平(天空不跟着滚转)。绕质心射线滚转会连带天空一起转,绕相机竖直轴则聚焦屏幕中心而非双指中心;dolly 必须先于 twist 执行(zoomEye 是事件起手位姿,后执行会覆盖拧转位移) |
| `onPinchEnd` |   .cpp:918   | **契约 2.3:双指 pan 无惯性**——不再种 pan 角速度;缩放动量足够时启动 zoom 惯性滑行。**2026-08-23:twist 惯性**——拧动速率 ≥0.05 rad/s 时启动;枢轴/轴与手势一致:有锚=锚点法线,无锚=指下地表点竖直轴(地图旋转语义,有界) |
| `tick` |   .cpp:966   | pan 惯性(视角角速度制,指数阻尼 **exp(-2.0/s)=iOS 0.998/ms**,停止判据用视角等效屏幕位移 <0.5px/帧;应用旋转按视差增益换算成绕地心角速度——修复前直接采绕地心率,低空被视差压小 ~1/4000,第一帧就判停) → 近地惯性(像素速度,同规则,无边界) → **滚轮平滑缩放(契约 3.1:指数收敛 ~300ms,单帧上限 ±ln(2),不越目标不反向)** → zoom 惯性(对数距离空间指数逼近,数学上永不越过锚点) → **twist 惯性(2026-08-23:有锚绕锚点法线、无锚绕指下地表点竖直轴;指数阻尼 2/s、0.5px/帧感知判停;视角角速度==轴角速度,无视差增益问题;每步 `clampNow` 防穿地)**。⚠️全部要求 `deltaSeconds > 0` ⇒ `tick(0.0)` 是完全空转 |
| `clampNow` | .cpp:1153 | `solver->constrainEye(userDriven=true, dt=0, anchor)` → 写回 → `commitPose`。恒 user-driven 是因为调用方刚刚显式动过相机 |
| `blendViewTowardGlobeCenter` | .cpp:1122 | **2026-08-19 新增,契约 2.4**:拉远到高空(≥1.5R)后,缩放路径把视轴按高度 smoothstep 转向地心,让球心自然回到屏幕中心(3.4R 偏移 393px→3px);4R 以上完全对准。只由缩放路径调用(捏合拉远/滚轮 settle/zoom 惯性),近地(<1.5R)不干预,平移/倾斜不抢方向 |
| `rotateCameraVerticalAroundPoint` |   .cpp:1168   | 绕 `camera_->right()` 在竖直面内转。三重守卫:up 翻转、`minSlope`、地形净空预判(用滤波高度,不重采样)。**拒绝而非事后顶起**——顶起要么破坏 Pitch 的锚点不变量,要么(Cesium 式旋转补偿)偷偷改 direction |
| `solveAnchorRotation` |   .cpp:1304   | 把「像素射线∩抓取球的点」转到 `anchorNormal` 的绕地心旋转 + 条件数 |
| `pointOnGrabSphere` |   .cpp:1342   | 真交点,或 miss 时取**最近接近点**(相切处与真交点重合 ⇒ 跨球缘 C0 连续) |
| `turntableDeltaFromPixels` |   .cpp:1367   | 转台回退:屏幕像素按 fov/height 换算角度(水平垂直同增益,aspect 抵消) |
| `tryAcquirePinchAnchor` |   .cpp:1390   | pick → 半径钳到 eye 以下(**防抓取球包住相机致射线命中背面疯转**)→ 方向换成射线∩钳位球(防起手跳变)。**2026-08-19 加两道拒绝**:掠射锚点(条件数<0.1)与远锚(距离>max(2×海拔,2km))起手即拒——低空近水平视线下远锚会让退化区"转台+整点重取"把锚点沿球面甩出(跳远);中途"每事件重试获取"同样受此守卫 |
| `latchPinchTwistPivot` | .cpp:1460 | **2026-08-23,无锚 twist 回退枢轴(起手 latch 一次)**:质心地表/椭球命中点 → 屏幕中心拾取点 → eye。轴 = 枢轴径向(当地竖直轴)——地图旋转语义:双指中心的地表点锚定钉在指下、地图绕它旋转、地平线水平;不逐事件重取,轴不漂移 |
| `applyPinchPin` |   .cpp:1502   | 把锚点钉到目标像素;病态区连续混入质心转台并整点重取锚点;末尾走 `clampNow`(pin 是唯一横向通道,山区横移可能把 eye 转进地形)。**契约 2.3:双指 pan 无惯性**,pin 增量不再累积速度。**2026-08-19:病态区转台经 `scaleTurntableToAnchor` 按锚点距/地心距缩放**——低空绕地心转台(手指角速度×R)单步可甩出数百公里(跳远),缩放到锚点尺度后相机位移≈角度×锚点距,高空自动退化为原转台 |
| `grabSurfacePoint` |  .cpp:1602  | 抓取锚点。⚠️锚点必须落在拾取射线上——生产 `pickTerrain` 已改射线行进(命中点在射线上,2026-08-19),此处 `pointOnGrabSphere` 重投影保留为对任意 surfacePicker 的防御(旧 off-ray 落差会被首个 move 一次性补掉 = 起手跳变,真机实测 227~471px)。**半径钳到 eye 以下**(锚点高于相机=仰视峰顶时防抓取球包住相机致射线命中背面疯转;低空朝下坡/崖时射线行进命中点在相机下方,不再触发钳制) |
| `applyAnchorDrag` |   .cpp:1641   | 良态区精确锚定;病态区 slerp 混入转台 + 应用后整点重取锚点(**永不渐近混合**——混出的锚点不属于任何真实几何,会积欠账);末尾按事件时间戳收集最近 ≤3 个惯性样本,松手时 iOS 权重 0.6/0.35/0.05 合成。**2026-08-19 修正:样本率 = 手指视角角速度**(原始像素位移×radPerPixel/dt),不是相机绕地心角速度——后者被视差压小,松手惯性第一帧就判停;并记录 `inertiaGain_`=|eye−anchor|/|eye| 供 tick 换算。**病态区转台同 `applyPinchPin` 经 `scaleTurntableToAnchor` 缩放**(防低空绕地心甩出) |

调优常量(.cpp:26 起,匿名 namespace):惯性 `kMaxInertiaAngularVelocityRadPerSec`=5(视角角速度上限)/ `kInertiaDampingPerSecond`=2(=iOS 0.998/ms)/ `kVelocitySmoothing`=0.35(zoom 惯性 EMA 用);近地 strafe `kNearModeMaxAltitudeMeters`=150000(Cesium) / `kNearGrazingDotThreshold`=0.05(Cesium `tangentPick`) / `kNearMinInertiaVelocityPxPerSec`=100(Mapbox+Flutter);`kTouchJerkLimit`=0.3 / `kMaxPinchScaleResidualLog`=1.0;`kTouchMinSlope`=0.1;`kPinchTiltRadiansPerPixel`=0.00873(=Mapbox 0.5°/px,保留质心绝对值映射);`kGrabSphereEyeMarginMeters`=25;病态带 `kAnchorConditioningLo/Hi`=0.10/0.35;`kMaxPinchAnchorAltitudeScale`=2.0 / `kMinPinchAnchorDistanceMeters`=2000(捏合锚点距离上限,海拔自适应);zoom 惯性 `kZoomInertiaDampingPerSecond`=6 / `kMaxZoomInertiaLogRate`=6 / `kMinZoomInertiaLogRate`=0.08;**twist 惯性(2026-08-23)`kTwistInertiaDampingPerSecond`=2(与拖拽同阻尼,Cesium inertiaSpin=0.9 等效 exp(-2.5t)) / `kMaxTwistInertiaRate`=2π / `kMinTwistInertiaRate`=0.05**;滚轮平滑(类内常量)`kZoomSettleRatePerSecond`=8(~0.3s 收敛 91%) / `kMaxZoomLevelsPerFrame`=1(单帧上限 ±ln(2),Mapbox maxScalePerFrame=2)。相机包络三常量(净空/最大地形高/地心距上限)不在这里,读 `CameraConstraintSolver`。

### camera/CameraPoseOps.h / .cpp

位姿旋转的两个自由函数(.h 29 行 / .cpp 33 行,`namespace camera_ops`)。
...
| `setRenderDevice` | .cpp:146-174 | Creates Renderer + SceneRenderPipeline, calls **`renderer_->initialize()` (no-arg)**, inits environment GPU resources; null device tears both down. **No globe mesh built or passed** — `GlobeMesh`/`Globe::createMesh` deleted |
| `update(dt)` | .cpp:180-262 | Phase 1. Builds `SceneFrameUpdateInput` via `frameRuntime_.makeFrameUpdateInput` and calls `SceneFrameUpdateCoordinator::update` (static) |
| `render()` | .cpp:302-328 | Phase 2. Guards on `renderer_`/`renderPipeline_`/`isReady()`; builds `SceneRenderPipeline::Context` from frameState + coordinator getters; `beforeSubmit` lambda = `updatePresentationTrace`; feeds result diagnostics back to telemetry |
| `interactionContext()` | .cpp:820-829 | Assembles `SceneInteractionContext` (camera, controller, primary tileset, vector layers) per call for pick/input via `frameRuntime_.makeInteractionContext` |
| Tileset API | .cpp:642-645 | `setTileset`→primary (+re-configures surface picker), `addTileset`→content; `hasTerrain()` = primary present |
| MVT runtime API | `.cpp:704-738` | `addMvtVectorSource` / `removeMvtVectorSource` / `mvtVectorLayer`; source and sink-bound `FeatureRenderLayer` are one Scene-owned lifecycle bundle. |
...
| **`kInteractionFocusTtlSeconds`** = 2.5 | .cpp:16-16 | Focus expires when `timeSeconds - focusTime > 2.5` |
| `updateInteractionFocus` | .cpp:36-50 | Latches focus dir only within TTL else zeros it |
...
| `update` | .cpp:97-143 | `primary_->update` timed → `terrainUpdateMs`; loops content → `contentTilesetUpdateMs`; Android logs any content tileset update > **5.0** ms |
...
| `configureCameraSurfacePicker` | .cpp:21-63 | Installs camera controller surface-picker (→pickInteractionFocus) + terrain-height func (→`SceneTerrainQuery::sampleHeight`) |
...
| `setupInputCallback` | .cpp:144-156 | InputManager gesture cb → `SceneInputCoordinator::handleGesture` |
...
Render flow in `render()` (.cpp:214-364):

| Order | Method | Lines | Builds |
|---|---|---|---|
| 0 | `reserveCommands` | .cpp:365-389 | Reserve = **4 + vectorLayers*4 + Σ tileset renderEntries** |
...
| 3 | `buildAtmosphereCommands` | .cpp:429-457 | AtmosphereBackgroundPass command from camera basis + sun dir + gradient params |
| 4 | `buildLayerCommands` | .cpp:458-746 | Inserts streamed tile cmds, then visible vector layers. **No fallback-globe command** — `makeGlobeCommand`/`GlobeSurface` removed; nothing is drawn if no tile commands exist。末尾还从**本帧可见地形瓦片包围体**汇总贴地高度范围喂给矢量层(O(可见瓦片数),零采样;头行 `clampH=min/max` 可读)。2026-09 热点④:拖动带新瓦时的成本在渲染器侧(掩码已改走 raster overlay,原 per-tile store 退役) |
| 5 | `applyMvpUniforms` | .cpp:798-806 | `SceneRenderCommandUniformUpdater::apply` |
| 6 | `sortAndValidate` | .cpp:807-852 | Sort if `mvpRenderOrder` inversion or translucent gltf; `updateSurfaceCommandGeneration`; `validateMvpRenderCommands` throws `std::runtime_error` on failure |
| 5.5 | `assembleTerrainBatches` | .cpp:747-797 | 地形实例化合批装配(资格闸 + 分组),`terrainBatcher_` |
| 6.5 | `prepareTerrainOcclusion` / `runTerrainDepthPrepass` | .cpp:853-882 / :883-931 | 地形遮挡参数下发 + 半分辨率地形深度 prepass(符号遮挡 T2) |
| 7 | `aggregateDiagnostics` | .cpp:932-965 | `SceneFrameDiagnosticsAggregator::aggregateRenderFrame` + terrain render-entry counters + `terrainSurfaceCommandsSubmitted` (`countTerrainSurfaceCommands`) |
| 8 | `shouldHoldPresentationAfterCommandBuild` | .cpp:966-1050 | 命令建完后是否压帧不呈现(hold 闸,见 presentation-hold 死锁那轮) |
| — | beforeSubmit → submit | .cpp:283-298 | `render()` 内:`beforeSubmit()`(presentation trace)→ `runTerrainDepthPrepass` → `renderer.submit(commands)`;随后 `releaseRenderReferences` 遍历全部 tileset(.cpp:1051-1089) |
...
| `mvpRenderOrder(kind)` | .h:309, .cpp:115-138 | Sky 0，glTF 15，atmosphere 20，普通矢量/Stencil 30，label 31，Unknown 100。 |
| `mvpRenderCommandLess` / `mvpRenderCommandsNeedSort` | .h:312-313, .cpp:143-150 | 复用统一比较器；相同 MVP pass 时先比较 `vectorPaintOrder`，因此跨瓦片/跨几何类型的矢量层级也能触发 fast-path 排序。 |
...
| Method | Lines | Notes |
| --- | --- | --- |
| `initialize()` (no-arg) | .h:34, .cpp:4376-4621 | Builds the neutral placeholder and compiles the terrain/glTF/vector shader set. glTF shader failure is non-fatal; terrain shader failure is fatal. |
| `submit` | .cpp:4622-4626 | Forwards to `device->submit` if initialized. |
| `dispose` | .cpp:4627-4693 | Resets the shader/texture resources. |
| Shared-resource getters | .cpp:4694-4714 | Depth/color/vector/atlas/placeholder/glTF/terrain resource accessors. Removed globe-resource getters remain absent. |
| `terrainShader()` | .h:123 / .cpp:4709-4714 | 32-byte terrain vertex, no PBR extensions; the draw side calls `makeTerrainPrimitiveCommand`. |
| `makeGltfPrimitiveCommand(vb,ib,idxCount,vtxCount)` | .cpp:4715-4740 | `GltfPrimitive`, **`vertexStride`=120**; defaults live in `GltfUniformBlock` and the factory does not populate the string-map. |
| `makeTerrainPrimitiveCommand(vb,ib,idxCount,vtxCount)` | .cpp:4741-4769 | Live compact-terrain command with **`vertexStride`=32**, `terrainShader`, and glTF uniform block enabled. |
| `makeGltfPrimitiveInstancedCommand(...)` | .cpp:4770-4790 | EXT_mesh_gpu_instancing path; delegates to the glTF command then installs the instance stream. |
| `makeTerrainInstancedCommand(...)` | .cpp:4791-4826 | Terrain batching path; delegates to the compact command then installs the terrain instance stream/shader. |
| `attachRasterInMainThread` / `detachRasterInMainThread` | .cpp:4827-4862 | No-op notification hooks; raster ownership stays in `DirectRasterMapping` / `SurfaceRasterBinding`. |
...
| 方法 | 行 | 说明 |
|---|---|---|
| `providerStackCompatibility` / `pageDomainCompatibilityName` | .cpp:630-669 / :698-728 | PageStore mixed-scheme 运行期契约:同一 compose group 必须共享 canonical page-facing scheme 与有效 projection；异构组整组 fail-closed |
| `pageSourceStackCompatibility` | .cpp:670-697 | 首 active source 必须是 opacity≈1 的 `BaseImagery`；annotation-only / translucent canonical base 整组 fail-closed。注意配置 opacity=1 尚不能证明源像素 alpha 全不透明。 |
| `beginSubmission(frameId)` / `SubmissionLease` | .cpp:1853-1861 | CPU command-consistency lease。SceneRenderPipeline 在命令构建前获取并持有到 submit/引用释放之后；期间 PageStore determination/tick/reconfigure/invalidate mutation 被拒绝。它不是 GPU completion lease。 |
| `initialize(device, Config)` | .cpp:1879-1933 | 建 `texture2DArray` + 间接纹理。**Config**:`pageSizeTexels`=256、`maxPages`=512、`maxUploadsPerFrame`=3、`maxComposeDispatchesPerFrame`=8 |
| `resetPageDomainState` / `rejectPageDomain` | .cpp:1934-1976 / :2218-2235 | canonical provider/scheme/projection 切换或拒绝时推进 generation，清空 scheme-less 页账本/层池/间接纹理；迟到异步结果由 generation 闸丢弃 |
| `updateVisiblePages(view, ...)` | .cpp:1045-2013 | **核心**。先做 provider/terrain page-domain 兼容性闸，再遍历可见瓦片 → 枚举 cell → 缺页则 `kickPageFetches` → 写间接纹理。合批资格闸也在这里(见下)。静止帧(视图签名+可见瓦片指纹+状态脏版本均未变)整段跳过 |
| `applyToTerrainCommand(cmd, tile)` | .cpp:2119-2195 | 仅 active+compatible domain 把该瓦片的间接层号/页参数写进地形 RenderCommand；拒绝态保留 directComposite |
| `tick()` | .cpp:2513-2582 | 每帧驱动:`drainInbox`(派发合成)+ `drainReadyUploads`(预算上传) |
| `drainInbox` / `kickPageFetches` | .cpp:2285-2373 / :2614-2702 | 解码结果派发 worker 合成，异步生产路径消费 PageStore `ComposeDispatch` grant；无 worker 的测试/同步 fallback 就地执行。页 fetch 消费 PageStore `NetworkRequest` grant，拒绝后保留待重试；两段均携带 page-domain generation |
| `drainReadyUploads` | .cpp:2636-2748 | worker 快照按本地预算与 PageStore `GpuUpload` grant 上传；上传前校验 page-domain generation + key + layer；`uploadedSources` 在此推进。grant 单位是逻辑事务，不等于底层写调用数 |
| `erasePageEntry` | .cpp:2045-2060 | 页换租/淘汰:cancel 在途 fetch + 移除账本 + **同步通知 decorator 释放**(不通知则被换租页的源数据成僵尸) |
| `resamplePageSource` | .cpp:729-797 | 源影像重采样进页(跨 zoom 档的 scale-bias) |
| `subtileGridN` / `enumerateSubtileKeys` (static) | .cpp:969-989 / :990-1010 | 瓦片 z 与源 zoom → 每边 cell 数;枚举 cell key |
| `placeTileInSourceGrid` (static) | .cpp:852-947 | 几何瓦片在**影像源瓦片网格**中的落位(x0/y0/cells + origin/span,单位=源瓦片)。cell 网格由几何等分改为源网格,让 GCJ-02 这类源网格不对齐的 overlay 也能走页存储;标准 overlay 恒退化成 `origin=0, span=gridN`(`isDegenerate`)= 零回归判据 |
| `encodeLayerRGBA8` / `decodeLayerRGBA8` / `decodeDepthRGBA8` (static) | .cpp:798-813 / :814-819 / :820-823 | 间接纹理的 RGBA8 编解码(层号 + resident 位 + 档位) |
| `packKey` / `unpackKey` (static) | .cpp:803-810 / :824-831 | TileKey ↔ uint64 页键；只在单 canonical domain 内有效，跨 domain 由 generation 隔离 |

GPU coverage 边界：`SubmissionLease` 只冻结 CPU 侧命令一致性。Metal `Renderer::submit()` 可能仍只编码，不能据此覆写 array layer；完整退场仍需 `RenderDevice submittedSerial/completedSerial → PageStore layer retireSerial`，completed serial 到达后才能重租/覆写。
...
| `attachRasterInMainThread` | .h:40-46 | cesium-native `attachRasterInMainThread`; called from `DirectRasterMapping::update()` Step 6 with geometry key, overlay slot, ready raster tile, GPU texture, UV translation/scale. |
...
| `beginFrame` / `beginRegion` / `endRegion` | .cpp:155-173 | 区间**平铺不嵌套**:开新区间即关上一个。TIME_ELAPSED 不可嵌套,而 EXT 的 TIMESTAMP 路径在多数 Adreno/Mali 上 `QUERY_COUNTER_BITS==0`(报支持、实则无效),故不做双路径分支 |
| `endFrame` | .cpp:180-208 | 收尾当前区间 + 采 `GL_GPU_DISJOINT_EXT` + 非阻塞回读最老的槽。**每帧只回读一个槽**,均摊 CPU |
| `tryResolve` | .cpp:209-254 | 全部 query 就绪才算完成(半帧的和偏小,而偏小的和最容易被读成「这个 pass 不贵」);同名段合并累加 |
...
**Stride-based vertex-layout dispatch** in `submit` (.cpp:1320-2041) — no VAOs; per-command `glVertexAttribPointer` keyed on `cmd.vertexStride`:
...
| Item | Lines | Description |
| --- | --- | --- |
| `makeDepthState` | .mm:158-180 | **Reverse-Z**: `MTLCompareFunctionGreaterEqual` (matches OpenGlobus `reverseDepth`); ctor precomputes readWrite/readOnly/disabled states (.mm:158-180) |
| Ctor | .mm:109-126 | Bridges `CAMetalLayer*`, creates command queue, depth states, `linearClampSampler` (maxAnisotropy=4) |
| Capabilities | .mm:224-247 | `maxTextureSize`=4096, `maxDrawBuffers`=4 hardcoded; `rendererString` from device name |
| `createTexture`/`updateTextureRegion` | .mm:248-374 | RGBA8/R8 pixel formats; per-texture sampler; `replaceRegion` bounds-checked |
| `createBuffer`/`updateBuffer` | .mm:375-421 | `MTLResourceStorageModeShared`; `updateBuffer` orphans storage to avoid CPU/GPU races |
...
| `beginFrame` | .mm:803-835 | `nextDrawable`; clearColor (0.1,0.3,0.6,1); **reverse-Z `clearDepth=0.0`**; bounded in-flight semaphore |
| `submit` | .mm:957-1375 | Per-command PSO/depth-state select; vertex buffer @0, instance buffer @7; glTF/terrain fragment uniforms use one mirrored `GltfUniformBlock` at buffer(0), vertex MVP at buffer(1), generic non-glTF uniforms retain small fixed slots; texture+sampler binding 0..kGltfWaterMaskTextureSlot; cull/blend; indexed/instanced draw |
| `endFrame` | .mm:1376-1407 | End encoding, `presentDrawable`, commit and publish completion serial |
| `onSurfaceChanged` | .mm:1424-1439 | Sets `drawableSize`; allocates `Depth32Float` depth texture (RenderTarget, Private) |
...
| `diagTag` / `fragForEffect` | .cpp:268-290 / :265-287 | Effect → 诊断名 / 片元源(tonemap/fog 由 PostProcessModel.h 生成函数拼接) |
| `initialize` | .cpp:291-333 | 编 shader,建全屏几何 |
| `ensureFramebuffer` | .cpp:334-368 | 按尺寸建/复用 FBO |
| `buildCommand` | .cpp:369-416 | 出后处理 RenderCommand |
| `dispose` | .cpp:417-424 | |
...
| `ensureTileProvider(RenderDevice*)` | .h:33 / .cpp:26-50 | lazily creates real provider; builds `RenderDeviceRasterTextureUploader` only if `device` non-null (.cpp:28-31); sets owner + throttle + SSE; then `syncProviderOptionsFromOverlay()` |
...
| `invalidateDirectExecutionState()` | .h:49 / .cpp:91-95 | backend generation 变化时取消 Direct waiter/source-set 并清理 mapped/direct tile 与 pending upload；保留 backend-neutral exact source cache |
| `setMaximumSimultaneousTileLoads(int)` | .h:52 / .cpp:105-115 | `n>0 ? n : 20`; applies to both providers |
| `getThrottledTilesCurrentlyLoading` | .h:53 / .cpp:117-120 | forward, 0 if none |
| `visible` / `opacity` | .h:55-56 / .cpp:122-128 | delegate to `overlay_` |
| `getProjection()` | .h:70 / .cpp:164-170 | **生效**采样投影(读 provider,不读 config)。georeference 会被 scheme 闸口拒掉,请求值不可信 |
| `syncProviderOptionsFromOverlay` (private) | .h:65 / .cpp:171-188 | re-derives throttle from overlay (`>0 ? : 20`), calls `applyOwnerOptions()` on both providers |
...
| 方法 | 行 | 说明 |
|---|---|---|
| `setStyle` | .cpp:667-681 | 换样式并标脏；越界表达式（含 paint order）在此剥离降级 |
| `makeClampSampler` / `prepareClampedFeature` | .cpp:923-949 / :862-942 | 贴地方案 A:边细分 + Steiner 采高；区域高度范围可让 worker 跳过逐点采样 |
| `syncDirtyBuckets` | .cpp:1035-1043 | 每帧入口:重建脏桶,返回重建数 |
| `tessellateFeatureInto` | .cpp:1044-1523 | 镶嵌总控；接收已解析的 paint ordinal，并分派 fill/line/extrusion/stencil |
| `appendFillVolume` / `appendLineVolume` | .cpp:2245-2419 / :2420-2672 | 面体 / 线体按 `(paintOrder,color)` 分组，避免贴地路径丢失逐要素层级 |
| `uploadBucketGpu` | .cpp:2673-2822 | 单几何类仍上传一对 VBO/IBO；paint ranges 保存 index offset/count，point/label 也按 ordinal 保存 ranges，stencil group 保存 ordinal |
...
| `buildRenderCommands` | .cpp:5305-5826 | 出命令总入口；同步当前 zoom 符号派生、字形预算与标签工作票 |
| `visibleBucketKeys` | .cpp:5827-5896 | 可见桶筛选 |
| `updateLabelPlacement` | .cpp:5897-6047 | 标签避让 + fade + 地平线剔除(P5c)。collect 含热点③ 屏外预剔除(culled 计数入哨兵日志) |
| `dumpLabelLifecycle` | .cpp:6154-6273 | 七态只读聚合 dump |
| `appendTerrainOcclusion` | .cpp:6294-6314 | 接地形深度 prepass 做符号遮挡(T2) |
| `appendBucketCommands` | .cpp:6315-7077 | 逐桶发命令；fill/line/extrusion/point/label ranges 与 stencil group 均写入 `vectorPaintOrder` |
| `beginEditPreview` / `updateEditPreview` / `endEditPreview` | .cpp:7078-7094 / :7095-7101 / :7102-7129 | 编辑预览三接口 |
| `pick` | .cpp:7173-7468 | 要素拾取；`ScenePickingCoordinator` 将结果转换为统一 `PickResult`，与 terrain/legacy vector 按相机距离取最近命中 |
...
| `tessellateFeature` | .h:97 / .cpp:244-267 | cache lookup, dispatch by `feature.type`, cache result |
| `tessellatePoint` | .h:100 / .cpp:268-285 | centroid = ECEF of first coord; placeholder vertex, quad indices (billboard built at draw time) |
| `tessellateLine` | .h:102 / .cpp:286-329 | subdivides each segment along ellipsoid (`subdivideArc`, 8 segs), LineStrip indices |
| `tessellatePolygon` | .h:104 / .cpp:125-180 | outer ring → ECEF, ear-clip `triangulatePolygon` |
...
| `extractRow` | .cpp:54-62 | 相对矩阵取行(96B 实例流的 rel×3) |
| `acquireInstanceBuffer` | .cpp:63-87 | 实例 buffer 池 |
| `rejectReasonName` | .cpp:88-105 | 理由名(诊断打印) |
| `batchTemplateGridIsUniform` / `firstMismatchedTemplateGrid` | .cpp:106-115 / :112 | **纯谓词、public** —— 故意暴露出来让单测不需要 GL 上下文就能验判据 |
...
| `ensureFramebuffer` | .cpp:44-76 | 半分辨率深度 FBO |
| `extractTerrainCommands` | .cpp:77-123 | 从主命令表抽地形命令(复用地形 VS + 空片元) |
| `depthTexture` | .cpp:124-128 | 供符号 shader 采样 |

### providers/VectorSurfaceFillRasterizer.h / .cpp

通用矢量面填充栅格化器(116 行)。把一组面要素(polygon)按给定 tile 矩形投影到
固定 256×256 RGBA 页。按 (drawOrder, identity) 分组求并集后做预乘 alpha 合成；
分组内用 `rasterizeAmapSurfaceMask` 的 even-odd coverage + 超采样反走样。与
具体数据源/样式解耦:颜色与身份由 `SurfaceFillResolver` 决定。是旧
`makeAmapSurfaceMaskImage` 的通用化版本,供任意矢量投影复用。

### providers/VectorSurfaceFillImageryProvider.h / .cpp

通用矢量面填充 ImageryProvider(108 行)。参数化 scheme/fetch/resolver/displayZoom;
`requestTile` → fetch 面要素 → `rasterizeSurfaceFill` 生成 256×256 页。
`setDisplayZoom` 每帧更新 packedState(低 8 位 = 离散 displayZoom);覆写
`targetSourceZoom()` 返回当前 displayZoom,使源 zoom 直接跟随相机(见
RasterOverlaySourcePlan.cpp 的 `chooseRasterOverlaySourceZoom`),近景无限细分,
独立于粗地形 tile zoom。
...
官方 classic-normal runtime 的唯一装配根。构造阶段安装 sealed style、assets 与 transport；`update` (.cpp:73-96) 每帧用真实相机高度算 displayZoom,喂给 surface-fill overlay provider(源 zoom 随相机细分),并推进官方 manifest、字体和 icon atlas 生命周期。调用方不能注入替代样式或 transport。
...
官方道路与沿线文字合同。`isAmapClassicTransportIdentity` (.cpp:227-231) 和 `isAmapClassicRoadLabelIdentity` (.cpp:287-325) 控制 admission；`applyRoadLabelPlacement` (.cpp:254-286)、`applyLineLabel` (.cpp:229-252)、`applyTransport` (.cpp:254-288) 消费生成表中的官方宽度、颜色、line type 与 zoom 条件。`paintOrder` 仅承载排序，不推导视觉 identity。

### style/AmapClassicLabelStyle.cpp

官方 POI label/icon 合同。`amapClassicPoiIconContractsValid` (.cpp:308-328) 校验生成表与 atlas 合同；`isAmapClassicPoiIdentity` (.cpp:248-272) fail-closed admission；`amapClassicPoiIconFrames` (.cpp:329-341) 与 `amapClassicPoiIconAtlases` (.cpp:273-285) 暴露 runtime-private atlas 清单；style contract 安装官方文字、halo、图标和 zoom 条件。
...
测试与 CPU page 生成辅助。official AMap scene 的普通面填充现已改走通用
`VectorSurfaceFillImageryProvider` raster overlay(见上),按相机 displayZoom
细分并叠加采样到地形,不再走 per-tile store 或 terrain shader 内合成。

### layers/ProjectedPathSampler.h / .cpp

沿线路径的屏幕空间采样器。构造阶段生成累计像素弧长并拒绝退化路径；`sample` (.cpp:135-205) 沿折线插值得到位置与切向，使道路文字按路径方向排布而非按首尾弦方向。
...
| Item | Lines | Description |
|---|---|---|
| `State` | .h:110-195 | LRU list+map、inflight waiters、命中/拉取计数、失败账本；shared_ptr 持有。`.cpp` 仅保留空 TU，模板实现都在头文件。 |
| `request` | .h:366-604 | 失败账本闸 / L1 命中 / 在途搭车 / L2 字节重解码 / 发起 fetch，并在完成后广播 waiters。 |
| `stats` | .h:608-615 | 累计命中、拉取、重复拉取、常驻瓦/字节和失败跳过快照。 |
...
高德 Nebula 解码几何到引擎 Feature 的规范化层。`AmapDecodedTileDecodeTraits::approxBytes` 给缓存预算近似值；`amapNormalizeEvenOddWinding` (.cpp:1038-1044) 统一 canonical top-down 环与 even-odd 嵌套；`amapClipPolygonRing` (.cpp:375-438) 裁剪越界环；`amapDecodedPartToFeatures`.cpp:332-43352) 完成坐标、属性、面/线/点转换。

### AmapVectorTile.h / .cpp

高德 gzip/protobuf 容器解码。`inflateContainer` 校验长度头并解压；`decodeInflatedContainer` (.cpp:261-494) 解析 layer/feature/geometry；`decodeAmapTile` (.cpp:53-99) 提供通用 type1/2/3/4 入口；`decodeAmapPoiTile` (.c.cpp:261-494在同一次 inflate 上追加权威 POI label 解码并过滤通用解码产生的伪 type-0 part。
...
把共享 `AmapDecodedTile` 路由到 regions/main/water/POI 四类 `VectorTileSourceT` 的 traits 与别名；过滤在消费阶段发生，使多个 source 共享同一份 gzip/protobuf 解码结果。`.cpp` 定义 `amapTileRectangle` (.cpp:126-137)，按高德 4326 等距圆柱 2:1 网格返回弧度矩形。
...
| `PolygonTessellator` (349) | `quantize`/`coordKey` (PolygonTessellator.cpp:22/:26)、`tessellate` (.cpp:23-26) | 面镶嵌(lng/lat CDT → ECEF;`maxEdgeMeters>0` 时边弧细分+内部 Steiner) |
| `LineTessellator` (94) | `dist3` (LineTessellator.cpp:12)、`tessellate` (LineTessellator.cpp:19) | 线镶嵌 |
| `StyleExpression` (207) | `literal` (StyleExpression.cpp:29/:36)、`literalString` (StyleExpression.cpp:44)、`get` (StyleExpression.cpp:51)、`zoom` (StyleExpression.cpp:58)、`match` (StyleExpression.cpp:64)、`lerpValues` (.cpp:13-30) | 样式表达式树。⚠️ `String` 必须用**独立命名**的 `literalString`,重载会歧义 |
...
| `MvtVectorSource` | `horizonViewRectangle` (MvtVectorSource.h:348)、`update` (MvtVectorSource.h:392)、`setLayerRules` (MvtVectorSource.h:796)、`ingestTileInbox` (MvtVectorSource.h:841) | 模板实现位于头文件；按视口拉瓦片并原子换手渲染集。 |
| `VectorTileTree` | `splitAntimeridian` (VectorTileTree.h:26)、`update` (VectorTileTree.h:147)、`provide` (VectorTileTree.h:526)、`provideShared` (VectorTileTree.h:532)、`markFailed` (VectorTileTree.h:550)、`zoomForCameraHeight` (VectorTileTree.h:601) | 模板瓦片树；R* 置换式细化，全有全无回退，renderTiles 保持精确覆盖。 |
...
| GLSL starfield vert/frag | .cpp:111-137 | `hash`/`noise3D`, `artStarLayer` 3 layers (.cpp:111-136), painted Milky-Way ribbon (.cpp:111-136); `u_nightFactor` scales stars & alpha (0=day transparent, 1=night) |
...
| **`kAuToMeters`** = 1.4959787e11 | .cpp:22-22 | AU → m |
...
| `process()` | .cpp:289-415 | 事件分发。**桌面绑定优先**:命中即整条消费,不漏到触摸路径(否则中键拖会同时 tilt 和拖地球) |
...
| `onSurfaceCreated()` | .h:45, .cpp:88-98 | `device_->onSurfaceCreated()` then `scene_->setRenderDevice(device_)`; sets `surfaceCreated_` on success. |
| `onSurfaceChanged(w,h,dpr=1)` | .h:48, .cpp:99-105 | Forwards to `device_->onSurfaceChanged` + `scene_->setViewport`. |
| `onSurfaceDestroyed()` | .h:51, .cpp:106-143 | `scene_->setRenderDevice(nullptr)` + `device_->onSurfaceDestroyed()`. |
...
| `onInputEvent(InputEvent)` | .h:62, .cpp:1153-1156 | Forward to `scene_->onInputEvent`. |
| `onDragStart/Move/End` | .h:65-67, .cpp:1157-1165 | Legacy compat: build `InputEvent` (PointerDown/Move/Up, `PointerType::Touch`) and call `onInputEvent`. |
| `addVectorLayer / removeVectorLayer / vectorLayerCount` | .h:72-78, .cpp:149-159 | Forward to scene_. |
| `setTileset(unique_ptr<Tileset>)` | .h:81, .cpp:1430-1438 | cesium-native aligned: unified terrain Tileset → `scene_->setTileset`. |
| `addTileset(unique_ptr<Tileset>)` | .h:83, .cpp:1448-1456 | Parallel 3D Tiles / glTF content Tileset; not terrain-sampled. |
...
| `hasTerrain()` | .h:94, .cpp:1474-1479 | `scene_->hasTerrain()`. |
...
| `getClearColor` | .cpp:1561-1568 | Reads `frameState().clearR/G/B/A`. |
| `diagnostics() / presentationTrace()` | .h:124-126, .cpp:1569-1572 / :1577-1580 | Runtime `Diagnostics` + per-frame `PresentationTrace`; `frameResourceArbiterSnapshot()` (1573-1576) sits between them and exposes producer×stage accounting. |
| `camera() / isReady()` | .h:130-131, .cpp:1182-1185, 1581-1585 | `isReady` = `scene_ && scene_->isReady()`. |
...
| `installScene(EarthSceneConfig)` | .h:37, .cpp:465-1009 | Move-stores config_, `resetCamera()`, clears overlay vectors, builds configured scene content, registers the AMap surface-fill raster overlay (follows camera display zoom), creates unified Tileset, optional glTF Tileset, sets sim time. |
| `resetCamera()` | .h:39, .cpp:1010-1093 | Rebuilds camera from `initialCamera` via `Ellipsoid::WGS84().cartographicToCartesian` + `geodeticSurfaceNormal`; `camera().lookAt(camEcef, targetEcef, up)`. No source rebuild. |
...
| `addActivatedRasterOverlay(...)` | .h:44-48, .cpp:1108-1124 | Wraps provider+scheme+options into `RasterOverlay`, then `ActivatedRasterOverlay`; pushes raw ptr into selection vector + owns both uniques. |
...
| GoogleMapTiles | `installScene` .cpp:690-768 | If no session, **blocking POST** `createSession` via `postBlocking` (.cpp:124) + `parseGoogleMapTilesCreateSessionResponse`; else reuse configured session. Default maxLevel 28. |
| Xyz (default/fallback) | .cpp:535-546 | `XYZImageryProvider` + XYZ-WebMercator. |

| Helper | Lines | Description |
| --- | --- | --- |
| `makeSceneTilesetOptions` | .cpp:77-94 | Maps `SceneTilesetConfig` → `TilesetOptions` (main-thread + cache-unload time limits). |
| `makeRasterOverlayOptions` | .cpp:96-116 | `RasterOverlaySourceConfig` → `RasterOverlay::Options` (loads/SSE/zoom/opacity/role/priority/fallback/blocks). |
| `postBlocking(...)` | .cpp:125-168 | **Blocking HTTP POST**: `PlatformBridge::post` + mutex/condition_variable, default timeout 20s; cancels request on timeout. |
| `applyConfiguredZoomRange` | .cpp:169-179 | Provider `setZoomRange`; no-op if both zooms ≤0. |
| `createTileSchemeForId` | .cpp:180-235 | `"XYZ-WebMercator"` → XYZWebMercator else GeographicTMS. |
| `SceneTerrainRuntimeSources` / `createTerrainRuntimeSources` | .cpp:236-305 | Terrain sources struct. None ⇒ empty; Heightmap builds the terrain provider/content chain and optional ellipsoid fallback. |
...
`initialize()` takes **no arguments** (Renderer.h:34) and compiles the shared terrain/glTF/vector shader set plus the shared tile IBO and placeholder texture (.cpp:4642-4664) — no globe buffers/shader, no `makeGlobeCommand`, no `RenderCommandKind::GlobeSurface`. `dispose()` mirrors it (.cpp:4613-4635). Full per-shader breakdown in §13.
...
`IPrepareRendererResources` (renderer/IPrepareRendererResources.h) is the renderer-decoupling interface (cesium-native `IPrepareRendererResources` equivalent) threaded through scene/tiling as `pPrepRenderer` (SceneTilesetCoordinator.cpp:55, SceneFrameRuntime.cpp:33, drain finalize `.h:247-250`). Renderer implements the notification hooks but **stores no imagery state**: `Renderer::attachRasterInMainThread` / `detachRasterInMainThread` are intentional no-ops (Renderer.cpp:4464-4428) — surface raster ownership lives in `DirectRasterMapping`/`SurfaceRasterBinding` (cesium-native `RasterMappedTo3DTile` equivalent). Raster attach happens on the **main thread** at upload-finalize time.
