...
| `build` | .cpp:809-911 | Per resource: chooses `makeGltfPrimitiveInstancedCommand` vs `makeGltfPrimitiveCommand` (.cpp:809-911); sets frame/gen, `terrainRenderContent` flag, `u_modelOrigin`=renderLocalOrigin, world sort center, opacity, surface-clip UV for terrain (.cpp:809-911); emits full PBR uniform block + texture-transform uniforms + 15-slot texture array (.cpp:809-911); water-mask slot `kGltfWaterMaskTextureSlot` (.cpp:809-911); binds up to `kMaxGltfRasterOverlays` overlay textures at `kGltfRasterOverlayTextureBase+i` with per-overlay UV/opacity/texCoord (.cpp:809-911); blend/depth for translucent (.cpp:809-911). |
