...
| **`docs/architecture/*`** | **子系统怎么搭的、为什么、接新功能从哪切** | **设计说明（行号无守卫，以符号名为准）** |

### 北极星文档（`docs/northstar/`）

现有 `vector.md` / `amap-vector.md` / `terrain.md` / `lighting.md` / `imagery.md`。其中 `amap-vector.md` 是高德供应商链路的专属北极星，引用 `vector.md` 的通用矢量契约，并作为高德协议、坐标和 source 细节的唯一北极星真源，避免这些内容在 MVT 通用文档中重复。每份用一张判据表钉住体验目标：

- **判据编号是跨会话稳定锚点**，只增不改。说「V5 不满意」即指该条，不必重新描述观感。
  命名空间：vector 用无前缀 `V*`/`P*`（历史既有），高德链路用 `A-*`，terrain 用 `T-*`，lighting 用 `L-*`，imagery 用 `I-*`。
...
八个核心子系统各一份「怎么搭的 / 关键决策为什么这么定 / 数据流 / 关键契约 / **诚实得失** / 扩展点 / 对照系」，
面向**未来接入新功能**。入口见 [`docs/architecture/README.md`](docs/architecture/README.md)（含子系统总图与跨模块诚实总账）。
引擎重心三大件 `tiling` / `terrain` / `renderer` 写得最深。**接某模块前先读它那份**；行号无守卫，以符号名为准。
