...
def parse(line):
    ...
    vpm = [float(x) for x in m.group(10).split(',')]
    return dict(phase=m.group(1), fx=float(m.group(2)), fy=float(m.group(3)),
                vw=int(m.group(4)), vh=int(m.group(5)), hasA=m.group(6)=='1',
                a=(float(m.group(7)), float(m.group(8)), float(m.group(9))), vpm=vpm)
...
for g in gestures:
    ...
    print(f"[{kind}] 帧={len(errs)}  anchorErr: 末={e[4]:.2f}px 峰={emax[4]:.2f}px"
          f"(@{emax[0]})  本该位移={fdisp:.1f}px 实际位移={adisp:.1f}px 增益={gain:.3f}")
