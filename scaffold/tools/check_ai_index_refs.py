...
# 允许"文件没有专属小节"的例外。**键 = 文件名,值 = 理由,理由不可省**。
# 当前为空:2026-08-06 把缺口清到 0(全部 89 个文件都有条目),阈值随之收到 0。
#
...
def main():
    ...
    # 默认 0 = **每个** .cpp/.mm 都必须有专属 ### 小节。
    # 2026-08-06 把缺口从 13 个(>300 行)一路补到 0 个(全部 89 个文件),
    # 阈值随之收到 0 —— 门槛留在 300 的话,新增的中小文件照样能静悄悄不进索引,
    # 而 TerrainPageStore 那次的教训正是"没人会主动去数文件"。
    ap.add_argument('--coverage-min-lines', type=int, default=0,
                    help='超过这么多行的 .cpp/.mm 必须有专属 ### 小节(默认 0 = 全部)')
