...
class QueuedStatusPlatformBridge : public PlatformBridge {
public:
    void onMemoryPressure() override {}
    void onEnterBackground() override {}
    void onEnterForeground() override {}
    ...
    bool completeNext(int statusCode,
                      std::vector<uint8_t> body = {}) {
        return completeAt(0, statusCode, std::move(body));
    }

    bool completeAt(size_t index,
                    int statusCode,
                    std::vector<uint8_t> body = {}) {
        std::function<void(int, std::vector<uint8_t>)> callback;
        {
...
TEST(QuantizedMeshTerrainProviderTest, HttpErrorFailsTerminally) {
...
TEST(QuantizedMeshTerrainProviderTest,
    ...
    ASSERT_TRUE(bridge.waitUntilPendingCount(1));
    ASSERT_TRUE(bridge.completeNext(200, bytes));
    {
        std::unique_lock<std::mutex> lock(mutex);
        ASSERT_TRUE(cv.wait_for(
            lock,
...
TEST(QuantizedMeshTerrainProviderTest,
     RequestTileContentOmitsRasterOverlayDetailsWhenAllChildrenAvailableLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/terrain/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest,
     InvalidParentLayerJsonRejectsAndPreservesStateLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    QueuedStatusPlatformBridge bridge;
    provider.setPlatformBridge(&bridge);

    const std::string stableLayerJson = R"json({
      "format": "quantized-mesh-1.0",
      "projection": "EPSG:4326",
    ...
    const std::string childLayerJson = R"json({
      "format": "quantized-mesh-1.0",
      "projection": "EPSG:4326",
      "tiles": ["child/{z}/{x}/{y}.terrain"],
      "parentUrl": "Parent",
      "maxzoom": 9
    })json";
    const std::string invalidParentJson = R"json({
      "format": "quantized-mesh-1.0",
      "projection": "EPSG:4326",
      "maxzoom": 12
    })json";
    ...
    ASSERT_TRUE(bridge.waitUntilPendingCount(1));
    EXPECT_EQ("https://terrain.example/root/Parent/layer.json",
              bridge.pendingUrl(0));
    ASSERT_TRUE(bridge.completeNext(
        200,
        std::vector<uint8_t>(
            invalidParentJson.begin(),
            invalidParentJson.end())));
    configThread.join();
...
TEST(QuantizedMeshTerrainProviderTest, PartialAvailabilityMatchesCesiumNativeLayerJson) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "tilejson": "2.1.0",
    ...
    EXPECT_TRUE(provider.supportsTile(TileKey{"Geographic-TMS", 2, 1, 0}));
    EXPECT_EQ(TileAvailabilityState::Available,
              provider.availabilityState(TileKey{"Geographic-TMS", 2, 1, 0}));
    EXPECT_FALSE(provider.supportsTile(TileKey{"Geographic-TMS", 2, 0, 0}));
    EXPECT_EQ(TileAvailabilityState::NotAvailable,
              provider.availabilityState(TileKey{"Geographic-TMS", 2, 0, 0}));
}
...
TEST(QuantizedMeshTerrainProviderTest, WebMercatorPartialAvailabilityUsesOneByOneRoot) {
    ...
    EXPECT_EQ(TileAvailabilityState::Available,
              provider.availabilityState(TileKey{"XYZ-WebMercator", 1, 1, 1}));
    EXPECT_FALSE(provider.supportsTile(TileKey{"XYZ-WebMercator", 1, 0, 0}));
    EXPECT_EQ(TileAvailabilityState::NotAvailable,
...
TEST(QuantizedMeshTerrainProviderTest, LayerJsonSchemeIsIgnoredLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.com/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, InvalidTilesRejectAndPreserveStateLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.com/fallback/{z}/{x}/{y}.terrain");
    const std::string validLayerJson = R"json({
      "attribution": "stable terrain credit",
    ...
    const std::string missingTilesLayerJson = R"json({
      "format": "quantized-mesh-1.0",
      "projection": "EPSG:4326",
      "scheme": "tms",
      "maxzoom": 9
    })json";
    const std::string emptyTilesLayerJson = R"json({
      "format": "quantized-mesh-1.0",
      "projection": "EPSG:4326",
      "scheme": "tms",
      "tiles": [],
    ...
    for (const std::string& invalidLayerJson :
         {missingTilesLayerJson, emptyTilesLayerJson, nonArrayTilesLayerJson}) {
        EXPECT_FALSE(provider.configureFromLayerJson(
            invalidLayerJson,
            "https://example.invalid/bad/layer.json"));
        EXPECT_EQ(stableUrl,
...
TEST(QuantizedMeshTerrainProviderTest, LayerJsonFormatIsOptionalAndIgnoredLikeCesiumNative) {
    QuantizedMeshTerrainProvider missingFormatProvider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string missingFormatLayerJson = R"json({
      "projection": "EPSG:4326",
      "scheme": "tms",
      "tiles": ["{z}/{x}/{y}.terrain"],
      "minzoom": 0,
    ...
    ASSERT_TRUE(missingFormatProvider.configureFromLayerJson(
        missingFormatLayerJson,
        "https://example.invalid/layer.json"));
    EXPECT_TRUE(missingFormatProvider.supportsTile(
        TileKey{"Geographic-TMS", 0, 0, 0}));
    EXPECT_EQ("https://example.invalid/0/0/0.terrain",
              missingFormatProvider.buildUrl(
                  TileKey{"Geographic-TMS", 0, 0, 0}));

    QuantizedMeshTerrainProvider unknownFormatProvider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string unknownFormatLayerJson = R"json({
    ...
    ASSERT_TRUE(unknownFormatProvider.configureFromLayerJson(
        unknownFormatLayerJson,
        "https://example.invalid/layer.json"));
    EXPECT_TRUE(unknownFormatProvider.supportsTile(
        TileKey{"Geographic-TMS", 0, 0, 0}));
    EXPECT_EQ("https://example.invalid/0/0/0.terrain",
              unknownFormatProvider.buildUrl(
                  TileKey{"Geographic-TMS", 0, 0, 0}));
}
...
TEST(QuantizedMeshTerrainProviderTest, MissingLayerVersionSubstitutesEmptyStringLikeCesiumNative) {
    ...
    EXPECT_EQ("https://example.invalid/1/1/0.terrain?v=",
              provider.buildUrl(TileKey{"Geographic-TMS", 1, 1, 0}));
}

TEST(QuantizedMeshTerrainProviderTest, LayerJsonExtensionsBuildQueryLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, WaterMaskOptionGatesExtensionQueryLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    provider.setWaterMaskEnabled(true);
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
      "projection": "EPSG:4326",
      "scheme": "tms",
...
TEST(QuantizedMeshTerrainProviderTest, UnknownAndMalformedPlaceholdersMatchCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, MissingParentLayerKeepsChildLayerLikeCesiumNative) {
    ...
    EXPECT_TRUE(provider.supportsTile(childTile));
    EXPECT_EQ(childBase + "/childTiles/1/0/0.terrain",
              provider.buildUrl(childTile));
    EXPECT_EQ(TileAvailabilityState::NotAvailable,
              provider.availabilityState(parentOnlyTile));
...
TEST(
    ...
    ParentLayerMetadataAvailabilityShadowsAvailableLikeCesiumNative) {
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() /
        "earth_md_qm_parent_metadata_shadows_available_test";
    std::filesystem::remove_all(root);
    std::filesystem::create_directories(root / "child");
    std::filesystem::create_directories(root / "parent");
    ...
    const std::vector<uint8_t> metadataTile =
...
TEST(QuantizedMeshTerrainProviderTest,
    ...
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() /
        "earth_md_qm_underlying_availability_reload_test";
    std::filesystem::remove_all(root);
    ...
    const std::filesystem::path childTilePath =
        root / "child" / "childTiles" / "0" / "0" / "0.terrain";
    const std::filesystem::path parentTilePath =
        root / "parent" / "parentTiles" / "0" / "0" / "0.terrain";
    writeBytes(childTilePath, makeQuantizedMeshBytesWithMetadata(""));
    writeBytes(parentTilePath, makeQuantizedMeshBytesWithMetadata(parentMetadata));
    ...
    auto loadRoot = [&provider, &rootKey]() {
        std::mutex mutex;
        std::condition_variable cv;
        bool done = false;
        TileContentLoadResult completed = TileContentLoadResult::retryLater();
    ...
    ProviderRequestDiagnostics requestDiag =
        provider.requestDiagnostics();
    EXPECT_EQ(2, requestDiag.requestsStarted);
    EXPECT_EQ(2, requestDiag.requestsCompleted);
    provider.applyAvailabilityUpdates(
    ...
    std::filesystem::remove(parentTilePath);

    TileContentLoadResult secondCompleted = loadRoot();
    EXPECT_EQ(TileLoadStatus::Renderable, secondCompleted.status);
    ASSERT_NE(nullptr, secondCompleted.gltfModel);

    for (int i = 0;
         i < 200 && provider.requestDiagnostics().requestsCompleted < 3;
         ++i) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    requestDiag = provider.requestDiagnostics();
    EXPECT_EQ(3, requestDiag.requestsStarted);
    EXPECT_EQ(3, requestDiag.requestsCompleted);
    EXPECT_EQ(0, requestDiag.requestsFailed);
    EXPECT_EQ(1, provider.estimatedRequestFanout(rootKey));
    EXPECT_TRUE(provider.supportsTile(parentOnlyChild));
...
TEST(QuantizedMeshTerrainProviderTest,
     AppliesUnderlyingAvailabilityEvenWhenCurrentTileFailsLikeCesiumNative) {
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() /
        "earth_md_qm_underlying_availability_failed_content_test";
    std::filesystem::remove_all(root);
...
TEST(QuantizedMeshTerrainProviderTest,
     UnderlyingAvailabilityCanCompleteBeforeContentLikeCesiumNative) {
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() /
        "earth_md_qm_underlying_availability_join_order_test";
    std::filesystem::remove_all(root);
...
TEST(QuantizedMeshTerrainProviderTest,
     ConcurrentUnderlyingMetadataRequestsShareInFlightLikeCesiumNative) {
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() /
        "earth_md_qm_shared_underlying_metadata_provider_test";
    std::filesystem::remove_all(root);
    ...
    std::vector<TileContentLoadResult> completed;

    auto callback = [&](const TileKey&, TileContentLoadResult result) {
        {
            std::lock_guard<std::mutex> lock(mutex);
    ...
    provider.requestTileContent(rootKey, CancellationToken{}, callback);
    provider.requestTileContent(rootKey, CancellationToken{}, callback);
    ...
    EXPECT_NE(std::string::npos,
              bridge.pendingUrl(1).find("parentTiles/0/0/0.terrain"));
    EXPECT_NE(std::string::npos,
              bridge.pendingUrl(2).find("childTiles/0/0/0.terrain"));
    ...
    EXPECT_EQ(3, provider.requestDiagnostics().requestsCompleted);
...
TEST(QuantizedMeshTerrainProviderTest,
     FailedUnderlyingMetadataMarksEmptySubtreeLoadedLikeCesiumNative) {
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() /
        "earth_md_qm_failed_underlying_metadata_provider_test";
    std::filesystem::remove_all(root);
    ...
    const TileKey rootKey{"Geographic-TMS", 0, 0, 0};
    const TileKey childAvailableKey{"Geographic-TMS", 1, 0, 0};
    const TileKey parentOnlyKey{"Geographic-TMS", 1, 1, 0};
    EXPECT_EQ(TileAvailabilityState::Unknown,
    ...
    EXPECT_EQ(TileLoadStatus::Renderable, completed.status);
    ASSERT_EQ(2u, completed.quantizedMeshAvailabilityUpdates.size());
    EXPECT_EQ(0, completed.quantizedMeshAvailabilityUpdates[0].layerIndex);
    EXPECT_EQ(1, completed.quantizedMeshAvailabilityUpdates[1].layerIndex);
    EXPECT_TRUE(
...
TEST(QuantizedMeshTerrainProviderTest, NonIntegerMaxzoomDefaultsToThirtyLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, LayerJsonMaxzoomDoesNotGateExplicitAvailabilityLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, AvailabilitySkipsNonArrayLevelsWithoutAdvancingLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, AvailabilitySkipsNonObjectRangesLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, AvailabilityUint32EdgeDoesNotOverflowLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, NegativeMetadataAvailabilityShadowsLayerAvailabilityLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
...
TEST(QuantizedMeshTerrainProviderTest, EmptyMetadataAvailabilityUpdateMarksSubtreeLoadedLikeCesiumNative) {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
