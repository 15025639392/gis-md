...
/// 编码逆变换:field byte → 到中心线距离(texel)。1=线心,0=远。
double distOf(uint8_t v) {
    return (1.0 - v / 255.0) * kLineFieldBandTexels;
}

} // namespace

// 水平中线:场值是**归一化中心线距离**的量化(宽度不进场,像素解算在
// FS)——这是"场线宽像素一致"编码的核心判据。线在 y=32.0,纹素中心
// +0.5 → 各行距离 = |row + 0.5 − 32|。
TEST(LineFieldRasterizerTest, StraightLineEncodesCenterlineDistance) {
    const MvtTile tile =
    ...
    // 邻行(中心偏 0.5 texel):dist ≈ 0.5。量化步进 band/255 给 ±0.2。
    EXPECT_NEAR(distOf(at(img, 32, 32)), 0.5, 0.2);
    // 中线 +4 行:dist ≈ 4.5。
    EXPECT_NEAR(distOf(at(img, 32, 36)), 4.5, 0.2);
    // 远处:0(band 外 clamp;0=失败安全的"无线")。
    EXPECT_EQ(at(img, 32, 8), 0);
    // band 边缘内一格(+7 行 → dist 7.5 < band 8):仍非 0。
    EXPECT_NEAR(distOf(at(img, 32, 39)), 7.5, 0.2);
    EXPECT_GT(at(img, 32, 39), 0);
}
...
TEST(LineFieldRasterizerTest, CrossingTakesMin) {
    ...
    EXPECT_GE(crossing, single)
        << "路口场值不得低于单线(min(dist) = max(编码))";
    EXPECT_GT(crossing, 191) << "路口贴近线心";
}

// 宽度**不烘进场**:不同 lineWidthPixels 产出逐字节一致(宽度语义整体
// 归 FS uniform;分级宽度载体在专项步 2 定案)。
...
TEST(LineFieldRasterizerTest, OverzoomSubRectPaintsLocalField) {
    ...
    const auto img = rasterizeLineFieldRect(
        ...
        lineStyle(8.0), 64);
    EXPECT_GT(at(img, 32, 32), 223) << "线心应在画布中央";
    EXPECT_EQ(at(img, 32, 8), 0) << "远处无线";

    // 不含线的子矩形(线在其下边界外):上半远区应全 0=无线
    // (下边缘可能落进编码带,不计)。
...
// 空瓦/无 line 层:全 0(远离一切线,失败安全),尺寸正确。
