...
void testQuantizedMeshLayerJsonTilesIgnoreNonStrings() {
    QuantizedMeshTerrainProvider provider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    const std::string layerJson = R"json({
      "format": "quantized-mesh-1.0",
    ...
    check(provider.configureFromLayerJson(
              layerJson, "https://example.invalid/layer.json"),
          "QuantizedMeshTerrainProvider: non-string tile templates are ignored like cesium-native");
    check(provider.buildUrl(TileKey{"Geographic-TMS", 1, 1, 0}) ==
              "https://example.invalid/valid/1/1/0.terrain",
          "QuantizedMeshTerrainProvider: first string tile template is used like cesium-native");
}
...
void testQuantizedMeshLayerJsonNonStringProjectionDefaults() {
    ...
    check(provider.configureFromLayerJson(
              layerJson, "https://example.invalid/layer.json"),
          "QuantizedMeshTerrainProvider: non-string projection layer configures like cesium-native");
    check(provider.schemeId() == "Geographic-TMS",
          "QuantizedMeshTerrainProvider: non-string projection defaults to EPSG:4326 like cesium-native");
...
void testQuantizedMeshLayerJsonRejectsUnknownProjection() {
    ...
    check(!provider.configureFromLayerJson(
              layerJson, "https://example.invalid/layer.json"),
          "QuantizedMeshTerrainProvider: unknown projection rejects like cesium-native Layer::getProjection");
    check(provider.schemeId() == "Geographic-TMS" &&
...
void testQuantizedMeshLayerJsonFailedConfigurePreservesPreviousState() {
    ...
    check(!provider.configureFromLayerJson(
              emptyTilesLayerJson,
              "https://example.invalid/empty-tiles/layer.json"),
          "QuantizedMeshTerrainProvider: empty tiles array rejects like cesium-native");
    check(provider.buildUrl(TileKey{"Geographic-TMS", 1, 1, 0}) == stableUrl &&
              provider.attribution() == "stable terrain credit" &&
              provider.supportsTile(TileKey{"Geographic-TMS", 0, 0, 0}) &&
              provider.maxZoom() == 4,
...
void testQuantizedMeshMetadataExtensionLengthPrefixMatchesCesiumNative() {
    ...
    check(missingAvailableOnly.empty(),
          "QuantizedMeshParser: metadata without available returns no availability like cesium-native");
    ...
    check(nonArrayAvailableOnly.empty(),
          "QuantizedMeshParser: metadata non-array available returns no availability like cesium-native");
...
void testQuantizedMeshMalformedMetadataStopsExtensionParsing() {
    ...
    check(mesh != nullptr,
          "QuantizedMeshParser: malformed metadata fixture remains parseable");
...
void testQuantizedMeshIncompleteExtensionHeaderStopsParsing() {
    auto scheme = TileScheme::createGeographicTMS();
    const TileKey rootKey{"Geographic-TMS", 0, 0, 0};
    const std::string metadata = R"json({
      "available": [
    ...
    std::vector<uint8_t> bytes = makeQuantizedMeshBytes(metadata);
    appendPod<uint8_t>(bytes, 99);
    appendPod<uint16_t>(bytes, 0x1234u);
...
void testQuantizedMeshLayerJsonVersionAndExtensionQuery() {
    ...
    QuantizedMeshTerrainProvider waterMaskOnlyProvider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    waterMaskOnlyProvider.setWaterMaskEnabled(true);
    ...
    check(waterMaskOnlyProvider.configureFromLayerJson(
              waterMaskOnlyLayerJson,
              "https://example.invalid/terrain/layer.json"),
          "QuantizedMeshTerrainProvider: watermask-only extension layer configures");
    check(waterMaskOnlyProvider.buildUrl(TileKey{"Geographic-TMS", 2, 3, 1}) ==
              "https://example.invalid/terrain/2/3/1.terrain?v=1.0.0&extensions=octvertexnormals-watermask",
          "QuantizedMeshTerrainProvider: watermask-only request does not add metadata like cesium-native");

    QuantizedMeshTerrainProvider disabledWaterMaskProvider(
        "https://example.invalid/fallback/{z}/{x}/{y}.terrain");
    ...
    check(disabledWaterMaskProvider.configureFromLayerJson(
              onlyWaterMaskLayerJson,
              "https://example.invalid/terrain/layer.json"),
          "QuantizedMeshTerrainProvider: disabled watermask-only extension layer configures");
    check(disabledWaterMaskProvider.buildUrl(TileKey{"Geographic-TMS", 2, 3, 1}) ==
              "https://example.invalid/terrain/2/3/1.terrain?v=1.0.0",
          "QuantizedMeshTerrainProvider: disabled watermask-only request omits extensions like cesium-native");
...
void testTileSelectionReusePolicyAllowsBoundedStaleReuseDuringSmoothing() {
    ...
    check(TileSelectionReusePolicy::classifyReuse(strictResourceChangedInput) ==
              TileSelectionReuseMode::None,
          "TileSelectionReusePolicy: equivalent selector views reselect after content resource refresh");
    check(TileSelectionReusePolicy::classifyReuseWithReason(
              strictResourceChangedInput).rejectReason ==
              TileSelectionReuseRejectReason::ResourceChanged,
          "TileSelectionReusePolicy: content resource refresh reports resource-changed reject reason");
...
void testProviderRequestDiagnosticsAggregatorCombinesCountsAndPeaks() {
    ...
    check(total.requestsStarted == 17 &&
              total.requestsCompleted == 15 &&
              total.activeWorkerBlockingRequests == 6 &&
              total.peakWorkerBlockingRequests == 8 &&
              total.maximumTransportActiveRequests == 11,
          "ProviderRequestDiagnosticsAggregator: combines provider request counts and max lanes");
    check(total.externalResourceRequestsStarted == 23 &&
              total.externalResourceRequestsCompleted == 20 &&
              total.activeExternalResourceBlockingRequests == 11 &&
              total.peakExternalResourceBlockingRequests == 9,
...
void testSceneDiagnosticsExposeTerrainSynchronousPrepReason() {
    ...
    Scene scene;
    check(scene.setRenderDevice(&device),
          "Scene: synchronous-prep diagnostics initialize renderer");
    scene.setViewport(800, 600, 1.0f);
    ...
    RasterOverlayTile* rootRaster =
        rootMapped ? rootMapped->getLoadingTile() : nullptr;
    check(rootRaster != nullptr,
          "Scene: synchronous-prep diagnostics root has base imagery");
    if (!rootRaster) return;
    rootRaster->setTexture(std::make_unique<DummyTexture>(4, 4));
    rootRaster->setMoreDetailAvailable(
        RasterOverlayTile::MoreDetailAvailable::No);
