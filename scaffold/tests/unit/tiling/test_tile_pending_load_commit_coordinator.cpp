...
TEST(TilePendingLoadCommitCoordinatorTest,
    ...
    update.metadataAvailability.push_back({1, 0, 0, 0, 0});
    provider->applyAvailabilityUpdates({update});

    TilesetTile* parent = contentAccess.ensureTile(parentKey);
    ASSERT_NE(nullptr, parent);
    parent->content.loadState = TileLoadState::ContentLoading;

    const std::string cacheKey = "terrain-non-boundary-retry-later";
    TileEmptyContentRegistry emptyContentRegistry;
    PendingTileLoad pending{
        TileLoadDomain::TerrainContent,
        parentKey,
    ...
    EXPECT_TRUE(resourcesDirty);
    EXPECT_EQ(TileLoadState::FailedTemporarily,
              parent->content.loadState);
    EXPECT_TRUE(parent->children.empty());

    const TileChildFrameMaterializeResult childResult =
        contentAccess.ensureTileChildren(*parent);

    EXPECT_TRUE(childResult.changed);
    EXPECT_FALSE(childResult.retryLater);
    ASSERT_EQ(4u, parent->children.size());
    EXPECT_EQ(availableChildKey, parent->children[0]->key);
    EXPECT_FALSE(parent->children[0]->content.isTerrainAvailabilityUpsample());
    for (size_t i = 1; i < parent->children.size(); ++i) {
        EXPECT_TRUE(
            parent->children[i]->content.isTerrainAvailabilityUpsample());
    }
    EXPECT_EQ(TileLoadState::FailedTemporarily,
              parent->content.loadState);
}
...
TEST(TilePendingLoadCommitCoordinatorTest,
     ContentUploadGeneratesRasterDetailsFromUpdatedContentBoundsLikeCesiumNative) {
    const TileKey key{"test", 0, 0, 0};
    const std::string cacheKey =
        "test:gltf-terrain-updated-content-bounds-raster";
    TilesetTile tile(key, Rectangle{});
    tile.content.loadState = TileLoadState::ContentLoading;

    const Rectangle originalTileRectangle =
        Rectangle::fromDegrees(-90.0, -45.0, 90.0, 45.0);
    const Rectangle originalContentRectangle =
        Rectangle::fromDegrees(-80.0, -30.0, 80.0, 30.0);
    tile.boundingVolume =
        TileBoundingVolume::fromRegion(originalTileRectangle, -100.0, 1000.0);
    tile.contentBoundingVolume =
        TileBoundingVolume::fromRegion(originalContentRectangle, -10.0, 120.0);

    const Rectangle updatedTileRectangle =
        Rectangle::fromDegrees(-40.0, -20.0, 40.0, 20.0);
    const Rectangle updatedContentRectangle =
        Rectangle::fromDegrees(-7.0, -3.0, 5.0, 9.0);
    TileLoadResultMetadata metadata;
    metadata.updatedBoundingVolume =
        TileBoundingVolume::fromRegion(updatedTileRectangle, -50.0, 500.0);
    metadata.updatedContentBoundingVolume =
        TileBoundingVolume::fromRegion(updatedContentRectangle, -5.0, 150.0);

    auto model = makeCommitCoordinatorQuadTerrainGltfModel(
        updatedContentRectangle);
    model->rasterOverlayDetails = RasterOverlayDetails{};
    TileLoadResult loadResult = TileLoadResult::fromContentResult(
        TileContentLoadResult::renderTerrain(
            std::move(model),
            std::move(metadata)));
    ASSERT_EQ(TileLoadStatus::Renderable, loadResult.status);
    ASSERT_TRUE(loadResult.isRenderableContentTerrain());
    ASSERT_FALSE(
        loadResult.content.metadata.rasterOverlayDetails.has_value());
    ...
    const Rectangle expectedContentWebMercator = projectRectangleSimple(
        ...
        updatedContentRectangle);
    const Rectangle updatedTileWebMercator = projectRectangleSimple(
        WebMercatorProjection(Ellipsoid::WGS84()),
        updatedTileRectangle);
    EXPECT_EQ(expectedContentWebMercator, *webMercator);
    EXPECT_NE(updatedTileWebMercator, *webMercator);
    EXPECT_EQ(0,
              committedDetails.textureCoordinateIDForProjection(
                  RasterOverlayProjection::WebMercator));
    ASSERT_TRUE(tile.boundingVolume.has_value());
    EXPECT_EQ(updatedTileRectangle, tile.boundingVolume->region);
    ASSERT_TRUE(tile.contentBoundingVolume.has_value());
    EXPECT_EQ(updatedContentRectangle, tile.contentBoundingVolume->region);
    EXPECT_TRUE(resourcesDirty);
    EXPECT_FALSE(lifecycle.containsWorkForCacheKey(cacheKey));
...
TEST(TilePendingLoadCommitCoordinatorTest,
     ContentUploadAppendsMissingRasterProjectionLikeCesiumNative) {
    const TileKey key{"Geographic-TMS", 2, 1, 1};
    const std::string cacheKey = "test:gltf-terrain-append-raster";
    const Rectangle bounds = Rectangle::fromDegrees(-12.0, -4.0, -6.0, 2.0);
    TilesetTile tile(key, bounds);
    tile.content.loadState = TileLoadState::ContentLoading;
    ...
    GltfModel* rawModel = model.get();
    ASSERT_EQ(1u, model->primitives.size());
    const std::vector<std::array<float, 2>> geographicTexCoords = {
        std::array<float, 2>{0.0f, 0.0f},
        std::array<float, 2>{1.0f, 0.0f},
        std::array<float, 2>{0.0f, 1.0f},
        std::array<float, 2>{1.0f, 1.0f}};
    model->primitives[0].vertexTexCoords[0] = geographicTexCoords;
    RasterOverlayDetails existingDetails;
    existingDetails.setGeographicRectangle(bounds, -25.0, 125.0);
    model->rasterOverlayDetails = existingDetails;
    ...
    ASSERT_NE(nullptr, committedModel);
    ASSERT_EQ(1u, committedModel->primitives.size());
    EXPECT_EQ(geographicTexCoords,
              committedModel->primitives[0].vertexTexCoords[0]);
    ASSERT_EQ(
        committedModel->primitives[0].vertices.size(),
        committedModel->primitives[0].vertexTexCoords[1].size());
    const RasterOverlayDetails& committedDetails =
        tile.content.renderContent.rasterOverlayDetails();
    ASSERT_EQ(2u, committedDetails.rasterOverlayProjections.size());
    ASSERT_EQ(2u, committedDetails.rasterOverlayRectangles.size());
    EXPECT_EQ(RasterOverlayProjection::Geographic,
              committedDetails.rasterOverlayProjections[0]);
    EXPECT_EQ(RasterOverlayProjection::WebMercator,
              committedDetails.rasterOverlayProjections[1]);
    EXPECT_EQ(bounds, committedDetails.rasterOverlayRectangles[0]);
    EXPECT_EQ(
        projectRectangleSimple(
            WebMercatorProjection(Ellipsoid::WGS84()),
            bounds),
        committedDetails.rasterOverlayRectangles[1]);
    EXPECT_EQ(0,
              committedDetails.textureCoordinateIDForProjection(
                  RasterOverlayProjection::Geographic));
    EXPECT_EQ(1,
...
TEST(TilePendingLoadCommitCoordinatorTest,
     TerrainContentUploadPreservesWebMercatorDetailsWhenAppendingGeographicLikeCesiumNative) {
    const TileKey key{"Geographic-TMS", 2, 1, 1};
    const std::string cacheKey =
        "test:gltf-terrain-preserve-webmercator-append-geographic";
    const Rectangle bounds = Rectangle::fromDegrees(-12.0, -4.0, -6.0, 2.0);
    TilesetTile tile(key, bounds);
    tile.content.loadState = TileLoadState::ContentLoading;
    ...
    GltfModel* rawModel = model.get();
    ASSERT_EQ(1u, model->primitives.size());
    const std::vector<std::array<float, 2>> webMercatorTexCoords = {
        std::array<float, 2>{0.125f, 0.875f},
        std::array<float, 2>{0.625f, 0.875f},
        std::array<float, 2>{0.125f, 0.375f},
        std::array<float, 2>{0.625f, 0.375f}};
    model->primitives[0].vertexTexCoords[0] = webMercatorTexCoords;
    const Rectangle webMercatorRectangle = projectRectangleSimple(
        WebMercatorProjection(Ellipsoid::WGS84()),
        bounds);
    RasterOverlayDetails existingDetails;
    existingDetails.rasterOverlayProjections = {
        RasterOverlayProjection::WebMercator};
    existingDetails.rasterOverlayRectangles = {webMercatorRectangle};
    existingDetails.rasterOverlayInvertedVCoordinates = {false};
    existingDetails.boundingRegion = {bounds, -25.0, 125.0};
    model->rasterOverlayDetails = existingDetails;
    ...
    ASSERT_NE(nullptr, committedModel);
    ASSERT_EQ(1u, committedModel->primitives.size());
    EXPECT_EQ(webMercatorTexCoords,
              committedModel->primitives[0].vertexTexCoords[0]);
    ASSERT_EQ(
        committedModel->primitives[0].vertices.size(),
        committedModel->primitives[0].vertexTexCoords[1].size());
    ...
    EXPECT_EQ(webMercatorRectangle,
              committedDetails.rasterOverlayRectangles[0]);
    EXPECT_EQ(bounds, committedDetails.rasterOverlayRectangles[1]);
    EXPECT_EQ(0,
              committedDetails.textureCoordinateIDForProjection(
                  RasterOverlayProjection::WebMercator));
    EXPECT_EQ(1,
...
TEST(
    ...
    ContentUploadUsesRasterOverlayDetailsToTightenLooseBoundsLikeCesiumNative) {
    const TileKey key{"test", 0, 0, 0};
    const std::string cacheKey = "test:gltf-terrain-raster-details-bounds";
    const Rectangle looseRectangle = Rectangle::MAXIMUM;
    TilesetTile tile(key, looseRectangle);
    tile.content.loadState = TileLoadState::ContentLoading;
    tile.boundingVolume =
...
TEST(
    ...
    const TileKey key{"test", 0, 0, 0};
    const std::string cacheKey =
        "test:gltf-terrain-invalid-raster-details-bounds";
    const Rectangle looseRectangle = Rectangle::MAXIMUM;
    TilesetTile tile(key, looseRectangle);
    tile.content.loadState = TileLoadState::ContentLoading;
    tile.boundingVolume =
    ...
    const Rectangle modelRectangle =
        Rectangle::fromDegrees(-11.0, -3.0, -5.0, 4.0);
    TileLoadResultMetadata metadata;
    metadata.rasterOverlayDetails.emplace();
    metadata.rasterOverlayDetails->rasterOverlayProjections.push_back(
        RasterOverlayProjection::Geographic);
    metadata.rasterOverlayDetails->rasterOverlayRectangles.push_back(
        modelRectangle);
    ...
    EXPECT_TRUE(tile.boundingVolume->region.equalsEpsilon(
        modelRectangle,
        1e-12));
    EXPECT_NEAR(-35.0, tile.boundingVolume->minimumHeight, 1e-5);
    EXPECT_NEAR(95.0, tile.boundingVolume->maximumHeight, 1e-5);
    const RasterOverlayDetails& details =
        tile.content.renderContent.rasterOverlayDetails();
    ASSERT_EQ(1u, details.rasterOverlayRectangles.size());
