...
CameraController::TerrainAreaSampleFunc makeFlatAreaSampler(
...
double nearFromGroundState(const CameraController& c) {
    return std::max(
...
void placeCamera(Camera& camera, CameraController& controller,
...
TEST(DynamicNearPlane, NearNeverClipsSampledTerrain) {
    ...
    for (double agl : agls) {
        for (double pitch : pitches) {
            ...
            EXPECT_GE(nearPlane, CameraController::kNearFloorMeters);
            // 平地最近几何 = 竖直 AGL；near 必须留出安全比余量。
            EXPECT_LE(nearPlane, CameraController::kNearSafetyRatio * agl +
                                     1e-6)
                << "agl=" << agl << " pitch=" << pitch;
        }
    }
...
TEST(DynamicNearPlane, NearFloorRespectsClearanceCoupling) {
    EXPECT_GE(CameraController::kNearSafetyRatio *
                  CameraController::kMinClearanceMeters,
              CameraController::kNearFloorMeters);
}
