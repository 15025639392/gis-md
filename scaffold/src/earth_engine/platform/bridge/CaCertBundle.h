...
// Mozilla CA 证书包(curl.se/ca/cacert.pem),用于 CURLOPT_CAINFO_BLOB。
// Android app 沙箱无法可靠使用系统 CA 目录(/system/etc/security/cacerts)
// —— vcpkg libcurl 的 OpenSSL 报 curl=60 "unable to get local issuer cert"
// (SELinux/哈希目录格式),故内嵌 CA 包用内存 blob 供证书校验,修复所有 HTTPS。
// 更新方式:重新下载 https://curl.se/ca/cacert.pem 覆盖本文件的 PEM 段。
namespace earth_engine {
inline constexpr char kCaCertBundlePem[] = R"CACERTPEM(
##
