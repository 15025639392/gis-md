...
/// 2026-08-21(L-P4)进一步:全部色板/数值参数抽到 SkyColorModel.h 的 C++
/// 常量,由 skyColorGLSLPreamble() 生成 GLSL 常量前缀,主体只引用常量名
/// —— 不存在第二处可漂移的 vec3 字面量;computeSkyColorCpu() 是同一公式
/// 的 host 镜像,供 ctest 采样对拍(改错不再是"真机肉眼才发现")。
inline std::string kSkyColorGLSL() {
    return skyColorGLSLPreamble() + sunsetWarmthRampGLSL() + R"(
vec3 computeSkyColor(vec3 rayDir, vec3 localUp, vec3 sun, float spaceFactor) {
    ...
    float skyT = smoothstep(kSkyGradientLo, kSkyGradientHi, viewUp);
    vec3 sky = mix(kSkyHorizon, kSkyZenith, pow(skyT, kSkyGradientPow));
    // 太空:大气变薄,整体压向近黑。
    sky = mix(sky, kSkySpace, spaceFactor);
    ...
    float lowSky = pow(1.0 - skyT, kSkyLowSkyPow);
    // 暖色**单一来源**(不引入平行 lowSkyColor 常数):日间→pale、日落→深橙。
    vec3 warm = mix(kSkyWarmDay, kSkyWarmSunset, sunLow);
    // 朝阳前向散射辉光:高日光紧致(pow8);日落时叠加展宽项(pow3)成朝阳侧橙霞
    // (pow3 比 pow2 更集中朝阳、反阳侧衰减更快)。
    float glow = pow(mu, kSkyGlowTightPow) * kSkyGlowTightScale +
                 pow(mu, kSkyGlowWidePow) * kSkyGlowWideScale * sunLow;
    sky = mix(sky, warm,
              clamp(glow * lowSky * (1.0 - spaceFactor), 0.0, kSkyGlowMixClamp));
    // 日落时整条地平线(各方位)再敷一层弱暖洗 → 日落氛围而非仅朝阳侧。lowSky
    // 已含幂收紧,幅度小(0.20)避免整片天变橙。
    sky = mix(sky, kSkyWashSunset,
              sunLow * lowSky * kSkyWashScale * (1.0 - spaceFactor));
    // 日落时天顶侧压向冷蓝紫,强化冷暖对比(仅高空 skyT、随 sunLow)。
    sky = mix(sky, kSkyZenithCool,
