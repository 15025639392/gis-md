...
// 地形约束侧的常量(净空/滤波/探针几何/锚点退出增益)全部归
// CameraConstraintSolver；这里只保留俯仰守卫做廉价净空预判要用的两个。
    ...
    // 要么(Cesium 式旋转补偿)偷偷改 direction,都不如"停住"。
    {
        ...
        if (hNext < kMaxTerrainHeightMeters + kMinAltitudeMeters) {
            const double minHeight =
                std::max(constraintSolver_.filteredTerrainHeight(), 0.0) +
                kMinAltitudeMeters;
            const double hCur =
...
glm::dquat CameraController::turntableDeltaFromPixels(double dx,
                                                      double dy) const {
...
bool CameraController::tryAcquirePinchAnchor(float xPixels, float yPixels) {
...
bool CameraController::grabSurfacePoint(float xPixels, float yPixels) {
    ...
    // 227~471px，海拔越低越大。方向取射线∩抓取球（miss 取最近接近点），
    // 于是 t=0 时锚点投影严格等于手指像素。
    bool trueHit = false;
