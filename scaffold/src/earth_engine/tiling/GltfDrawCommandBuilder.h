...
struct GltfDrawCommandBuildContext {
    ...
    // Fill mask is owned by the selected tile, even when this command renders
    // an ancestor renderTile footprint. It is intentionally separate from the
    // height/clip descendant so ancestor UV remapping cannot affect mask UVs.
